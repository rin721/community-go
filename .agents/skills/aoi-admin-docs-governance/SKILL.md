...
2. 用 `rg` / `rg --files` 查真实入口、脚本、配置、路由、测试和已有引用；不要只根据旧文档改文档。
3. 区分文档类型：
...
- 文档描述当前行为，不描述已删除的插件系统、旧前端入口、旧命令、旧配置块或尚未实现的生产能力。
...
2. 搜索旧说法在 README、docs、AGENTS、脚本 README 和 release 文档中的残留。
3. 用当前事实替换旧说法；若能力缺失，写入 `docs/backlog/known-gaps.md`，不要写成已完成。
4. 对历史证据类文档只在确有必要时更新，并保留其时间语境。
...
3. 不恢复 `docs/ai`、根 `ai` 或 `.ai` 作为分散规则入口。
