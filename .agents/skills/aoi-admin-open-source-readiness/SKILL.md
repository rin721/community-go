...
2. 用 `rg` 查目标关键词、入口边界、扩展边界、不可复用默认值、README 链接、任务计划入口、发布证据和脚本 gate。
3. 区分受控品牌叙事、运行时可配置品牌、审计证据文档、当前事实和未来缺口；不要把阶段证据误改成最新状态，也不要把未补证事项写成已完成。

## 审查口径

- 根 README 的 Aoi 项目代号、徽章、Logo 和标星曲线是受控品牌叙事例外；运行时代码、配置默认值、API、日志、错误信息、部署示例和前端生产文案仍应可配置、可替换。
- 业务扩展走 `internal/modules` 和模块化注册；配置、API、前端入口和示例围绕模块 contract 展开。
...
2. 按问题类型修复：品牌/入口收敛、扩展边界、文档链接、README 覆盖、错误结果边界、部署防线、release evidence、视觉 QA 或任务计划。
