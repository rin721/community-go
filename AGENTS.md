...
涉及产品范围、兼容性、migration、历史数据或模块保留/移除的任务，Agent 必须先读取根 README 的“当前项目目标语境”和 [项目范围与当前状态](docs/repository-scope.md)。不得把 `origin/main`、历史 migration 或本地运行数据自行等同于正式发布和外部兼容承诺；也不得以“首发前”为由跳过本地数据检查与破坏性操作确认。项目阶段变化只更新上述 authority，本文件继续只保留这条强制判定规则。
...
- 实现通用能力、接入外部系统或继续扩展现有自研能力前，必须先分类当前实现属于标准库、成熟第三方、项目特有逻辑还是自研通用机制，并调研 Go 标准库、项目现有能力和维护成熟的第三方技术栈；存在合适方案时不得无理由重复造轮子，也不得因已有自研代码而默认继续扩建。
- 技术选型至少评估功能覆盖、维护活跃度、版本演进、许可证、安全记录、生产采用证据、Go 版本兼容性、API 稳定性、可测试性、可观测性、生态兼容性、扩展能力和长期替换成本，不能只依据流行度、示例数量或当前代码改动最小。
- 重要候选必须使用当前官方源码、官方文档、release、安全公告或标准复核维护状态和适用范围；已废弃、长期无人维护、存在未处置已知漏洞或不兼容当前工具链的方案不得作为默认选项。
- 现有架构不是技术选型的不可变前提。若其职责边界、生命周期、装配方式或扩展模型阻碍成熟方案合理接入，研究必须比较“继续适配现状”与“重构承载架构”的实际收益；不得为了兼容历史结构持续叠加 Adapter、兼容层、特殊分支或隐藏回退。
- 架构调整和依赖替换都必须有可验证收益。不得为了追求更新、统一、设计模式或表面解耦而进行无收益重构，也不得在缺少真实用例、迁移边界和验证计划时引入重量级框架。
- 第三方技术跨越业务或稳定项目边界、承担资源生命周期、错误/安全语义或未来替换价值时，必须通过项目自有窄契约、Adapter 或明确基础设施边界接入；纯局部实现细节可以直接依赖合适第三方，不得机械地为每个库复制一层接口。
...
- 没有合适成熟方案，或成熟方案的复杂度、约束和引入成本经证据证明高于项目收益时才允许自研；自研前必须说明候选不适用的证据、项目特有价值、维护责任、风险边界、退出条件和验证计划，重要能力应记录设计决策。
...
- `<seq-num-name>` 使用仓库内递增三位序号和语义名称。每次建立新方案都必须同时产出需求、设计和任务，固定包含 `README.md`、`research/`、`requirements/`、`design/` 和 `tasks.md`：
  - `README.md` 标明变更范围、当前状态和阅读顺序。
  - `research/` 保存本任务可检索、可复核的结构化研究档案，格式与复用方式见 [研究档案与报告](docs/research/README.md)。
  - `requirements/` 是面向客户和产品使用方的需求产物。`requirements/README.md` 只负责需求摘要、阅读导航和文档边界；至少再包含一份按业务语义命名的需求明细，说明客户目标、使用场景、范围、约束、非目标和可验收行为。需求不得混入内部类名、包路径、函数调用链等实现设计；确有外部可见技术约束时，应翻译为客户能够判断的能力、限制或风险。
  - `design/` 是面向技术人员的详细设计产物。`design/README.md` 只负责设计摘要、关键决策和阅读导航；至少再包含一份按技术主题命名的详细设计。详细设计应按任务适用性说明模块边界、接口、数据与控制流、状态和生命周期、资源所有权、错误与失败语义、并发与安全、配置与迁移、文件影响和验证方案，不得只用概述、任务清单或文件列表代替设计。
  - 需求或设计包含多个可独立理解的主题时，必须继续拆成语义明确的 Markdown 文件，并由各自 `README.md` 串联；不得把大量内容重新堆入目录入口，也不得使用 `part-1.md`、`misc.md` 等无语义名称。
  - `tasks.md` 是唯一完成清单。它使用稳定任务 ID，并以 Markdown checkbox（`- [ ]` / `- [x]`）记录研究、计划、实施和验证任务；每项写明依赖或前置条件、完成条件和必要证据。只有完成条件已满足且证据已记录的任务才能标记为 `- [x]`，表格中的“完成”文字不能代替 checkbox 状态。
- 上述目录结构适用于本规则生效后新建的方案；历史变更保留原结构作为证据，不做无收益批量迁移。历史方案产生实质性新方案时，应以新的变更序号按当前结构记录，不在旧单文件中继续堆叠。
...
- 研究门禁通过后，其证据快照、适用边界和刷新触发器构成实施基线。从计划进入实施不得自动重开研究，也不得因阶段切换而重复全量阅读、搜索、技术选型或代码调用链追踪。
...
4. 当任务新增业务或应用模块、通用能力、外部系统接入，或继续扩展现有自研通用机制时，必须先按项目文档识别现有能力、依赖边界、资源所有权、生命周期与基础契约适配性，并比较成熟候选与当前实现；研究同时回答“实现是否适合”和“当前架构是否适合作为承载基础”。项目没有可验证的接入路径、现有契约无法表达真实需求，或承载架构会迫使方案增加无价值兼容层时，停留在研究和计划阶段，先补齐项目级设计。具体组件、目录、候选和门禁由项目文档定义，本文件不展开。
...
1. 研究门禁通过后，在 `docs/changes/<seq-num-name>/` 完成 `requirements/`、`design/` 和 `tasks.md`，并在需求入口、设计入口和任务清单中引用支撑结论的研究 ID；非纯文档任务把状态标为“待确认”，纯文档任务明确记录其例外和直接实施范围。
...
4. 涉及技术选型、通用能力或架构调整的计划必须逐项标明保留、升级、替换、合理自研、退役或架构重构结论，给出明确候选技术、项目职责边界、接入位置、迁移/退出条件和验证证据；不得只罗列库名或用“兼容当前实现”代替收益判断。
5. 计划报告之前的请求、其他任务的确认、历史消息中的概括授权都不能跨任务复用。

### 4.4 确认与实施阶段

- 只有研究门禁已经通过，且用户在计划报告之后的后续消息中明确确认当前任务及当前计划，才能把任务状态改为“已确认”并开始实施。
- 用户要求调整计划时，先更新任务文档并重新提交报告；不得把修改意见本身视为实施确认。
- 实施入口只做增量基线检查：记录当前 revision 与 Git 状态，将研究快照之后的变更路径与已计划文件、研究边界和 `refresh_triggers` 比较。没有相关变更或未命中触发器时，记录“基线未漂移”并直接执行已确认任务；禁止设置泛化的“重新核对当前代码事实”任务。
- 只有相关变更命中已记录的刷新触发器，或直接证据表明研究前提可能已失效时，才打开受影响的证据与必要代码做定向复核。定向复核确认前提未变时继续实施；推翻前提时才返回研究阶段。
- 已计划的失败用例、红绿测试、运行态复现和修改后回归属于实施与验证，不是第二轮方案研究。它们可以验证已确认结论，但不得无触发条件地扩展成全仓代码审计或重做技术选型。
