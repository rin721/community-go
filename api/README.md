...
当前公开契约聚合 Auth、IAM、Organization、Navigation 与 Todo。Auth 提供低敏审计只读查询（`auth.audit.list`，`auth:audit:read`）——subject/resource 只返回摘要，不做删除/篡改，支持按 operation/action/resourceType/outcome/actorKind/时间窗过滤；该查询同时承载 065 业务操作审计事件（IAM/Organization/Navigation 写操作经窄 port 注入同一审计面，不新增公共写 operation）。IAM 拥有账号、角色、权限、账号会话集中管理、机器访问令牌与 MFA/TOTP operation，使用 `iam:account:*`、`iam:role:*`、`iam:session:*`、`iam:api-token:read/write` 与自助键（`iam:account:self:*`、`iam:api-token:*`）等精确权限；066 起提供账号/角色资料更新与归档（复用 `iam:account:write`/`iam:role:write`，归档为终态）；会话列表/吊销（`iam.sessions.list/revoke`）只暴露 SessionID 摘要、支持分页与 status 过滤；076 起增加影响分析反向查询与账号多维过滤；078 起增加 API-Token 管理面（`iam.api-tokens.list/create/rotate/revoke`，access token 明文只在创建/轮换响应一次）与 MFA 自助绑定/登录第二步（`iam.login.mfa-verify`，`mfa_required` 挑战 + TOTP/恢复码）；080 起扩展令牌生命周期（`iam.api-tokens.update/disable/enable` 与 `status` 过滤参数，权限知情创建：token scope 受限于创建者当前有效权限）；084 起增加账号批量操作（`iam.accounts.status.batch` 与 `iam.accounts.archive.batch`，`POST /api/v1/iam/accounts/batch-status|batch-archive`，`iam:account:write`）：逐账号复用既有安全语义（owner 不变量/安全 revision/session 撤销/审计），单个失败不中止，响应返回 `processed/failed` 与逐条稳定错误码。Organization 拥有部门、岗位和账号组织分配 operation，使用 `organization:department:*` 与 `organization:position:*` 精确权限；分配写入使用 `expectedVersion` 乐观锁（冲突 409）；组织关系只作为目录数据，不进入 Auth decision。076 起 Organization 全部 operation 使用 `webuiSession` 认证 profile，mutation 请求要求 Origin 与 Session 绑定的 CSRF token（与 Navigation 一致，经 composition 注入的 IAM 守卫校验）。Navigation 使用 `navigation:menu:read/write` 管理已注册菜单策略，修改请求使用 `webuiSession`、Origin 与 Session 绑定的 CSRF token；它不提供动态 Route 或第二套角色菜单授权。
