...
5. 浏览器契约快照：`go run ./cmd/app webui generate` 从本文件渲染 `webui/src/generated/openapi-spec.ts`（JSON 对象，供 Admin WebUI 的 openapi 模块用 Swagger UI 可视化渲染，075）。该快照是同一产物的 JSON 变换，不是第二份权威；`webui generate --check` 严格比对，避免与契约漂移。
