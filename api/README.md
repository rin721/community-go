...
当前公开契约聚合 Auth、IAM、Organization、Navigation 与 Todo。Auth 提供低敏审计只读查询（`auth.audit.list`，`auth:audit:read`）——subject/resource 只返回摘要，不做删除/篡改，支持按 operation/action/resourceType/outcome/actorKind/时间窗过滤；该查询同时承载 065 业务操作审计事件（IAM/Organization/Navigation 写操作经窄 port 注入同一审计面，不新增公共写 operation）。IAM 拥有账号、角色、权限与账号会话集中管理 operation，使用 `iam:account:*`、`iam:role:*`、`iam:session:*` 等精确权限；066 起 IAM 还提供账号/角色资料更新与归档（`iam.accounts.update/archive`、`iam.roles.update/archive`，复用 `iam:account:write`/`iam:role:write`），归档为终态、不可登录与分配；会话列表/吊销（`iam.sessions.list/revoke`）只暴露 SessionID 摘要、支持分页与 status（all/active/revoked）过滤，并沿用安全修订与 owner 不变量；076 起 IAM 增加影响分析反向查询（`iam.roles.accounts.list`、`iam.permissions.roles.list`，复用 `iam:role:read`/`iam:permission:read`）与账号列表多维过滤（status/archived/roleId）。Organization 拥有部门、岗位和账号组织分配 operation，使用 `organization:department:*` 与 `organization:position:*` 精确权限；分配写入使用 `expectedVersion` 乐观锁（冲突 409）；组织关系只作为目录数据，不进入 Auth decision。076 起 Organization 全部 operation 使用 `webuiSession` 认证 profile，mutation 请求要求 Origin 与 Session 绑定的 CSRF token（与 Navigation 一致，经 composition 注入的 IAM 守卫校验）。Navigation 使用 `navigation:menu:read/write` 管理已注册菜单策略，修改请求使用 `webuiSession`、Origin 与 Session 绑定的 CSRF token；它不提供动态 Route 或第二套角色菜单授权。
