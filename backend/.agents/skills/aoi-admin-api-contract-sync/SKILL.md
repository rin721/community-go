...
3. 在 `internal/transport/http/contracts.go` 增加或修改 route contract：
   - method
   - Gin 风格 path
   - access level
   ...
   - path/query 参数
4. 如影响菜单权限，确认 System 菜单权限能在 API catalog 中找到同 `productCode + scope + permission` 声明。
5. 更新前端 `web/app/app/lib/api` endpoint 表和调用代码。
6. 生成 `docs/api/openapi.yaml`，不要手写生成产物。
7. 同步 API 文档、模块文档、权限矩阵和测试。
