...
先克隆仓库:
  ...
  cd <repo-address>
  bash deploy.sh --docker y --image console-platform:local --confirm

直接安装:
  curl -fsSL -o deploy.sh https://raw.githubusercontent.com/<owner>/<repo>/main/deploy.sh
  bash deploy.sh --repo https://github.com/<owner>/<repo>.git --docker y --image console-platform:local --confirm

GitHub 代理安装:
  curl -fsSL -o deploy.sh https://raw-githubusercontent-com-gh.helloworlds.eu.org/<owner>/<repo>/main/deploy.sh
  bash deploy.sh --github-proxy-host github-com-gh.helloworlds.eu.org --repo-slug <owner>/<repo> --docker y --image console-platform:local --confirm
...
应用选项:
  --db-driver <value>
  --db-sqlite-path <value>
  --db-mysql-host <value>
  --db-mysql-port <value>
  --db-mysql-username <value>
  --db-mysql-password <value>
  --db-mysql-database <value>
  --db-mysql-charset <value>
  --db-postgres-host <value>
  --db-postgres-port <value>
  --db-postgres-username <value>
  --db-postgres-password <value>
  --db-postgres-database <value>
  --db-postgres-ssl-mode <value>
  --db-pool-max-open-conns <value>
  --db-pool-max-idle-conns <value>
  --cache-driver <disabled|local|redis|hybrid>
  --cache-local-max-cost <value>
  --cache-local-num-counters <value>
  --cache-local-buffer-items <value>
  --cache-local-default-ttl-seconds <value>
  --cache-redis-addr <value>
  --cache-redis-username <value>
  --cache-redis-password <value>
  --cache-redis-db <value>
  --cache-redis-pool-size <value>
  --cache-redis-min-idle-conns <value>
  --cache-redis-max-retries <value>
  --cache-redis-dial-timeout <value>
  --cache-redis-read-timeout <value>
  --cache-redis-write-timeout <value>
  --server-host <value>
  --server-port <value>       容器内 HTTP 端口，同时写入 APP_SERVER_PORT，默认 9999。
  --server-mode <value>
  ...
  --brand-product-name <value>
  --brand-product-code <value>
  --brand-version-name <value>
  --log-level <value>
  --log-format <value>
  ...
  --storage-local-public-url <value>
...
APP_CONTAINER_PORT="${APP_CONTAINER_PORT:-${APP_SERVER_PORT:-9999}}"
...
APP_ENV_PREFIX="${APP_ENV_PREFIX:-APP}"
...
while [ "$#" -gt 0 ]; do
	...
	--db-sqlite-path)
		require_arg "$1" "${2:-}"
		set_app_env DB_SQLITE_PATH "$2"
		shift 2
		;;
	--db-mysql-host)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_HOST "$2"
		shift 2
		;;
	--db-mysql-port)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_PORT "$2"
		shift 2
		;;
	--db-mysql-username)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_USERNAME "$2"
		shift 2
		;;
	--db-mysql-password)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_PASSWORD "$2"
		shift 2
		;;
	--db-mysql-database)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_DATABASE "$2"
		shift 2
		;;
	--db-mysql-charset)
		require_arg "$1" "${2:-}"
		set_app_env DB_MYSQL_CHARSET "$2"
		shift 2
		;;
	--db-postgres-host)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_HOST "$2"
		shift 2
		;;
	--db-postgres-port)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_PORT "$2"
		shift 2
		;;
	--db-postgres-username)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_USERNAME "$2"
		shift 2
		;;
	--db-postgres-password)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_PASSWORD "$2"
		shift 2
		;;
	--db-postgres-database)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_DATABASE "$2"
		shift 2
		;;
	--db-postgres-ssl-mode)
		require_arg "$1" "${2:-}"
		set_app_env DB_POSTGRES_SSL_MODE "$2"
		shift 2
		;;
	--db-pool-max-open-conns)
		require_arg "$1" "${2:-}"
		set_app_env DB_POOL_MAX_OPEN_CONNS "$2"
		shift 2
		;;
	--db-pool-max-idle-conns)
		require_arg "$1" "${2:-}"
		set_app_env DB_POOL_MAX_IDLE_CONNS "$2"
		shift 2
		;;
	--cache-driver)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_DRIVER "$2"
		shift 2
		;;
	--cache-local-max-cost)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_LOCAL_MAX_COST "$2"
		shift 2
		;;
	--cache-local-num-counters)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_LOCAL_NUM_COUNTERS "$2"
		shift 2
		;;
	--cache-local-buffer-items)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_LOCAL_BUFFER_ITEMS "$2"
		shift 2
		;;
	--cache-local-default-ttl-seconds)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_LOCAL_DEFAULT_TTL_SECONDS "$2"
		shift 2
		;;
	--cache-redis-addr)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_ADDR "$2"
		shift 2
		;;
	--cache-redis-username)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_USERNAME "$2"
		shift 2
		;;
	--cache-redis-password)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_PASSWORD "$2"
		shift 2
		;;
	--cache-redis-db)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_DB "$2"
		shift 2
		;;
	--cache-redis-pool-size)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_POOL_SIZE "$2"
		shift 2
		;;
	--cache-redis-min-idle-conns)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_MIN_IDLE_CONNS "$2"
		shift 2
		;;
	--cache-redis-max-retries)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_MAX_RETRIES "$2"
		shift 2
		;;
	--cache-redis-dial-timeout)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_DIAL_TIMEOUT "$2"
		shift 2
		;;
	--cache-redis-read-timeout)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_READ_TIMEOUT "$2"
		shift 2
		;;
	--cache-redis-write-timeout)
		require_arg "$1" "${2:-}"
		set_app_env CACHE_REDIS_WRITE_TIMEOUT "$2"
		shift 2
		;;
	--server-host)
		require_arg "$1" "${2:-}"
	...
	--brand-product-name)
		require_arg "$1" "${2:-}"
		set_app_env BRAND_PRODUCT_NAME "$2"
		shift 2
		;;
	--brand-product-code)
		require_arg "$1" "${2:-}"
		set_app_env BRAND_PRODUCT_CODE "$2"
		shift 2
		;;
	--brand-version-name)
		require_arg "$1" "${2:-}"
		set_app_env BRAND_VERSION_NAME "$2"
		shift 2
		;;
	--log-level)
		require_arg "$1" "${2:-}"
	...
	--storage-local-base-path)
		...
		shift 2
		;;
	--storage-local-public-url)
		require_arg "$1" "${2:-}"
		set_app_env STORAGE_LOCAL_PUBLIC_URL "$2"
		shift 2
		;;
...
if [ -n "$SOURCE_DIR" ]; then
	[ -f "$SOURCE_DIR/Dockerfile" ] || die "--source-dir does not look like a console platform checkout"
	...
	cleanup_dir="$(mktemp -d "${TMPDIR:-/tmp}/console-platform.XXXXXX")"
