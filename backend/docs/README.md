...
| 扩展方式 | 通过 `internal/modules` 新增业务模块，并接入模块化交付链路 |

## 维护原则

- 文档只描述当前代码、当前入口、当前扩展路径和已暴露的生产能力。
