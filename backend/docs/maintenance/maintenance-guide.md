...
维护工作应保持代码、测试、文档、Agent 规则、技能和验证证据一致。根目录 `AGENTS.md` 是唯一项目级长期规则入口；`.agents/skills` 只存可复用执行技能，`tools/ai` 只存工具配置，`tmp/ai` 只存可清理的本地临时证据。

## 常规变更流程

1. 阅读 `AGENTS.md`、相关目录 README、对应专题文档和任务触发的 `.agents/skills`。
2. 识别本次变更真实影响的代码边界。
3. 更新代码和测试。
4. 更新 `docs` 下的结构化人类文档。
5. 如果验证证据、发布证据或维护结论变化，更新 `docs/testing`、`docs/release`、`docs/maintenance` 或 `tmp/ai` 下的临时证据。
6. 根据影响范围运行定向测试和更广测试。
7. 将剩余风险记录到 `docs/backlog/known-gaps.md`、发布证据或维护审计中。
...
- 当前状态必须能从代码、文档、脚本和命令输出复原，不得依赖聊天历史作为唯一证据。
