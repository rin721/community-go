...
| 社区公开视频举报 | `POST /api/v1/public/community/videos/:idOrSlug/reports` | 公开 | 无权限码 | `frontend/` 视频页稿件举报弹窗 | 以匿名 `clientId` 写入后端待处理举报记录；审核处理、误报关闭和通知需另行设计后台权限与状态 |

Community 模块当前提供公开读取、轻量公开评论 / 弹幕发布、匿名创作者关注、匿名视频互动和视频级举报提交，供 Nuxt 视频社区在关闭 mock 后接入。真实投稿、评论 / 弹幕审核、举报处理后台、登录态关系归并、通知和创作者后台管理还不是后端生产能力，前端相关能力只能作为后端不可用时的浏览器降级或后续任务说明。
