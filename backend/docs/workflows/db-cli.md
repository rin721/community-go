...
命令由 `cmd/console` 声明为 `cli.CommandSpec`，通过 `pkg/cli` 的 Cobra 封装解析参数。

## 示例

```bash
go run ./cmd/console db --operation=database --print-sql
go run ./cmd/console db --operation=database --apply
...
go run ./cmd/console db migrate down --config=configs/config.yaml
```
...
- 命令声明保持在 `cmd/console`，使用 `cli.CommandSpec`/`cli.FlagSpec` 描述命令和 flag。
...
- 数据库连接关闭失败、迁移结果写入 stdout 失败和 SQL 执行失败都必须返回给 CLI 调用方；这些失败不属于可静默忽略的 best-effort 行为。
