...
type TrafficProbeRunner struct {
	...
	logger   TrafficProbeWarningLogger
}

// TrafficProbeWarningLogger 是探针适配器 best-effort 序列化降级时使用的最小日志端口。
type TrafficProbeWarningLogger interface {
	Warn(string, ...interface{})
}

type TrafficProbeRunnerOption func(*TrafficProbeRunner)

func NewTrafficProbeRunner(options ...TrafficProbeRunnerOption) TrafficProbeRunner {
	runner := TrafficProbeRunner{Resolver: net.DefaultResolver}
...
func WithTrafficProbeLogger(logger TrafficProbeWarningLogger) TrafficProbeRunnerOption {
	return func(runner *TrafficProbeRunner) {
...
func (r TrafficProbeRunner) Probe(ctx context.Context, target model.TrafficProbeTarget) model.TrafficProbeResult {
	...
	if err != nil {
		return r.failedTrafficProbe(target, "url", "invalid target url", err, start)
	}
	...
	if err != nil {
		return r.failedTrafficProbe(target, "request", "invalid probe request", err, start)
	}
	req.Header.Set("User-Agent", "console-platform-traffic-probe/1.0")
	resp, err := client.Do(req)
	if err != nil {
		return r.failedTrafficProbeWithTrace(target, recorder, "request", "probe request failed", err, start)
	}
	...
	if issue := validateTrafficProbeResponse(target, resp, &result); issue != nil {
		...
		result.EvidenceJSON = r.evidenceJSON(evidence)
		return result
	}
	if strings.TrimSpace(target.ExpectedContentKeyword) != "" && target.Method != model.TrafficProbeMethodHEAD {
		...
		if err != nil {
			return r.failedTrafficProbeWithTrace(target, recorder, "body", "read response body failed", err, start)
		}
...
func (r TrafficProbeRunner) failedTrafficProbe(target model.TrafficProbeTarget, stage string, reason string, err error, start time.Time) model.TrafficProbeResult {
	return r.failedTrafficProbeWithTrace(target, &trafficProbeTrace{}, stage, reason, err, start)
}

func (r TrafficProbeRunner) failedTrafficProbeWithTrace(target model.TrafficProbeTarget, recorder *trafficProbeTrace, stage string, reason string, err error, start time.Time) model.TrafficProbeResult {
	return model.TrafficProbeResult{
		...
		TTFBMs:            recorder.ttfbMs(),
		TotalDurationMs:   time.Since(start).Milliseconds(),
		EvidenceJSON: r.evidenceJSON(map[string]any{
			"dnsIps": recorder.ips(),
			"error":  errString(err),
			"stage":  stage,
		}),
...
func (r TrafficProbeRunner) evidenceJSON(value any) string {
	raw, err := json.Marshal(value)
	if err != nil {
		if r.logger != nil {
			r.logger.Warn("traffic probe evidence marshal failed", "error", err)
		}
		return "{}"
	}
	return string(raw)
