...
func TestTrafficProbeRunnerSuccessWithContentKeyword(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte("console expected marker"))
	}))
	defer server.Close()
...
func TestTrafficProbeRunnerEvidenceJSONWarnsOnMarshalFailure(t *testing.T) {
	logger := &captureProbeWarningLogger{}
	runner := NewTrafficProbeRunner(WithTrafficProbeLogger(logger))

	got := runner.evidenceJSON(map[string]any{"bad": func() {}})
	if got != "{}" {
		t.Fatalf("evidenceJSON() = %q, want {}", got)
	}
	if len(logger.entries) != 1 {
		t.Fatalf("warning count = %d, want 1", len(logger.entries))
	}
	entry := logger.entries[0]
	if entry.message != "traffic probe evidence marshal failed" {
		t.Fatalf("warning message = %q, want traffic probe evidence marshal failed", entry.message)
	}
	if len(entry.keysAndValues) != 2 || entry.keysAndValues[0] != "error" {
		t.Fatalf("warning fields = %#v, want error field", entry.keysAndValues)
	}
}
...
type captureProbeWarningLogger struct {
	entries []probeWarningEntry
}

type probeWarningEntry struct {
	message       string
	keysAndValues []interface{}
}

func (l *captureProbeWarningLogger) Warn(message string, keysAndValues ...interface{}) {
	l.entries = append(l.entries, probeWarningEntry{
		message:       message,
