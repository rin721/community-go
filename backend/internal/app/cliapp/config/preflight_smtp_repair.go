...
func promptSMTPPreflightRepair(ctx *cli.Context, configPath string, cfg *appconfig.Config) (bool, error) {
	...
	case preflightSMTPActionDebug:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.smtp.info.debug"), "write SMTP preflight debug notice"); err != nil {
			return false, err
		}
		return true, nil
	case preflightSMTPActionFile:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.smtp.info.file"), "write SMTP preflight file notice"); err != nil {
			return false, err
		}
		return true, nil
	case preflightActionRuntimeEnvOnly:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.smtp.info.runtimeEnv"), "write SMTP preflight runtime env notice"); err != nil {
			return false, err
		}
		return true, nil
