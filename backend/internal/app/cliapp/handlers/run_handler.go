...
import (
	...
	initservice "github.com/open-console/console-platform/internal/app/cliapp/services/init"
...
var inspectInitializationStatus = initservice.InspectInitializationStatus
...
func (h *RunHandler) RunStartFlow(ctx *cli.Context) error {
	...
	if updates.HasChanges() {
		...
		if err := writePromptInfo(ui, localizer.T("cli.run.info.privacyHandled"), "write privacy update confirmation"); err != nil {
			return err
		}
	}
	return h.startServer(ctx, configPath)
...
func (h *RunHandler) handleInitializationBeforeServer(ctx *cli.Context, ui cli.PromptUI, configPath string, cfg *appconfig.Config) (bool, error) {
	localizer := localization.FromContext(ctx)
	status, err := inspectInitializationStatus(ctx.Context, configPath)
	if err != nil {
		return false, fmt.Errorf("inspect initialization status before starting server: %w", err)
	}
	if !status.Required {
		return false, nil
	}
	if ctx.GetBool("yes") {
		if err := writePromptInfo(ui, localizer.T("cli.run.info.uninitializedWithYes"), "write uninitialized startup notice"); err != nil {
			return true, err
		}
		return false, nil
	}
	...
	case "web":
		...
		if err := clioutput.PrintServiceState(ctx.Stdout, state, localizer); err != nil {
			return true, err
		}
		if _, err := fmt.Fprintln(ctx.Stdout, localizer.T("cli.run.setupUrl", map[string]any{"URL": setupURL(cfg)})); err != nil {
			return true, fmt.Errorf("write setup URL: %w", err)
		}
		return true, nil
	case "status":
		return true, printInitializationStatus(ctx.Stdout, status, localizer)
	case "skip":
		if err := writePromptInfo(ui, localizer.T("cli.run.initialization.skipped"), "write initialization skip notice"); err != nil {
			return true, err
		}
		return false, nil
	default:
...
func printInitializationStatus(stdout io.Writer, status initcenter.Status, localizer *localization.Localizer) error {
	if stdout == nil {
		return nil
	}
	...
	if err := writeInitializationStatus(stdout, localizer.T("cli.run.initialization.status.line", map[string]any{"Required": status.Required, "Completed": status.Completed, "CurrentStep": current})); err != nil {
		return err
	}
	if status.RestartRequired {
		if err := writeInitializationStatus(stdout, localizer.T("cli.run.initialization.status.restartRequired", map[string]any{"Reason": strings.TrimSpace(status.RestartReason)})); err != nil {
			return err
		}
	}
	for _, step := range status.Steps {
		...
		if err := writeInitializationStatus(stdout, "- "+localizer.T("cli.run.initialization.status.failedStep", map[string]any{"Title": step.Title, "Key": step.Key})); err != nil {
			return err
		}
		if step.ErrorMessage != "" {
			if err := writeInitializationStatus(stdout, "  "+localizer.T("cli.run.initialization.status.reason", map[string]any{"Reason": step.ErrorMessage})); err != nil {
				return err
			}
		}
		if step.RepairHint != "" {
			if err := writeInitializationStatus(stdout, "  "+localizer.T("cli.run.initialization.status.repairHint", map[string]any{"Hint": step.RepairHint})); err != nil {
				return err
			}
		}
	}
	if status.Report != nil {
		...
		if err := writeInitializationStatus(stdout, localizer.T("cli.run.initialization.status.report", map[string]any{"Successful": report.Successful, "Failed": report.Failed, "Skipped": report.Skipped, "Risk": report.Risk})); err != nil {
			return err
		}
	}
	return nil
...
func writeInitializationStatus(stdout io.Writer, line string) error {
...
func (h *RunHandler) startServer(ctx *cli.Context, configPath string) error {
	...
	return clioutput.PrintServiceState(ctx.Stdout, state, localization.FromContext(ctx))
}
