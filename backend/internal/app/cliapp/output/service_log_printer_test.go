...
func TestPrintServiceLogsReturnsWriteError(t *testing.T) {
	writeErr := errors.New("stdout unavailable")
	err := PrintServiceLogs(context.Background(), outputErrorWriter{err: writeErr}, managed.ServiceState{}, 2, false)

	if !errors.Is(err, writeErr) {
		t.Fatalf("PrintServiceLogs() error = %v, want %v", err, writeErr)
	}
	if !strings.Contains(err.Error(), "write service logs") {
		t.Fatalf("PrintServiceLogs() error missing write context: %v", err)
	}
}

func TestPrintNewLogContentReturnsWriteError(t *testing.T) {
	writeErr := errors.New("stdout unavailable")
	...
	_, err := printNewLogContent(outputErrorWriter{err: writeErr}, "stdout", path, 0)

	if !errors.Is(err, writeErr) {
		t.Fatalf("printNewLogContent() error = %v, want %v", err, writeErr)
	}
	if !strings.Contains(err.Error(), "write service logs") {
		t.Fatalf("printNewLogContent() error missing write context: %v", err)
	}
}
