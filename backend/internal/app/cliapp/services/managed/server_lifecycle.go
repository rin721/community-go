...
func (m *Manager) StartServer(ctx context.Context, configPath string) (ServiceState, error) {
	...
	if err != nil {
		...
		if stateErr := m.writeState(state); stateErr != nil {
			return state, managedStateWriteError(err, stateErr)
		}
		return state, err
	}
	...
	if err := m.removeControl(ServiceServer); err != nil {
		failedAt := m.now()
		state.Status = StatusFailed
		state.LastError = err.Error()
		state.StoppedAt = &failedAt
	...
	alive, err := m.runner().IsProcessRunning(info)
	if err != nil {
		verifyErr := fmt.Errorf("verify managed service process after startup: %w", err)
		failedAt := m.now()
		state.Status = StatusFailed
		state.LastError = verifyErr.Error()
		state.StoppedAt = &failedAt
		if stateErr := m.writeState(state); stateErr != nil {
			return state, managedStateWriteError(verifyErr, stateErr)
		}
...
func (m *Manager) Status(ctx context.Context, service string) (ServiceState, error) {
	...
	if state.PID <= 0 || !ActiveStatus(state.Status) {
		return m.withUnmanagedServerStatus(state)
	}
...
func (m *Manager) withUnmanagedServerStatus(state ServiceState) (ServiceState, error) {
	if state.Service == "" {
	...
	if err != nil || !ok || details.PID <= 0 {
		if err != nil {
			return state, fmt.Errorf("detect unmanaged server listener on %s: %w", state.ListenAddr, err)
		}
		return state, nil
	}
	...
	if !looksLikeConsoleServerProcess(details, state.ConfigPath) {
...
func looksLikeConsoleServerProcess(details cliappadapters.ProcessDetails, configPath string) bool {
	...
	if strings.Contains(executable, "console") || strings.Contains(command, "cmd/console") || strings.Contains(command, " console") {
		return true
	}
...
func managedStateWriteError(cause error, stateErr error) error {
	if stateErr == nil {
		return cause
	}
	wrappedStateErr := fmt.Errorf("persist managed service state: %w", stateErr)
	if cause == nil {
		return wrappedStateErr
	}
	return errors.Join(cause, wrappedStateErr)
}
