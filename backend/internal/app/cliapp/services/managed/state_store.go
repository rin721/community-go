...
func (m *Manager) writeState(state ServiceState) error {
	...
	if err := removeManagedStateFile(path); err != nil {
		return withManagedStateTempCleanup(err, tmp)
	}
	if err := os.Rename(tmp, path); err != nil {
		return withManagedStateTempCleanup(fmt.Errorf("replace managed service state file: %w", err), tmp)
	}
	return nil
}
...
func (m *Manager) removeControl(service string) error {
	path := m.controlPath(service)
	if err := os.Remove(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("remove managed service control file: %w", err)
	}
	return nil
}

func removeManagedStateFile(path string) error {
	if err := os.Remove(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("remove managed service state file: %w", err)
	}
	return nil
}

func withManagedStateTempCleanup(primary error, tmp string) error {
	if cleanupErr := removeManagedStateTempFile(tmp); cleanupErr != nil {
		return errors.Join(primary, cleanupErr)
	}
	return primary
}

func removeManagedStateTempFile(path string) error {
	if err := os.Remove(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("remove managed service state temp file: %w", err)
	}
	return nil
}
