...
func (s *Service) Status(ctx context.Context) (Status, error) {
	...
	bootstrap, restartRequired, err := s.pendingRestart()
	if err != nil {
		return Status{}, err
	}
	restartReason := ""
...
func (s *Service) Schema(ctx context.Context) (SetupSchema, error) {
	...
	for _, def := range s.definitions() {
		...
		inputFingerprint, err := inputFingerprintFor(schema.Key, schemaVisibleValues(schema))
		if err != nil {
			return SetupSchema{}, err
		}
		schema.InputFingerprint = inputFingerprint
		...
		if err := s.runStep(ctx, exec, def); err != nil {
			if run != nil {
				...
				if saveErr := s.store.saveRun(ctx, run); saveErr != nil {
					result, resultErr := s.result(ctx, exec)
					return result, errors.Join(err, saveErr, resultErr)
				}
			}
			result, resultErr := s.result(ctx, exec)
...
func (s *Service) SaveConfig(ctx context.Context, stepKey string, input Input, values map[string]any, persist bool, runTest bool) (ConfigSaveResult, error) {
	...
	if saved.InputFingerprint == "" {
		inputFingerprint, err := inputFingerprintFor(stepKey, values)
		if err != nil {
			return ConfigSaveResult{}, err
		}
	...
	if s.store != nil {
		...
		if _, err := s.store.upsertStep(ctx, run, def, StepStatusPending, 0, saved.InputSummary, nil); err != nil {
			return ConfigSaveResult{}, err
		}
		records, err := s.store.steps(ctx, run.ID)
		if err != nil {
			return ConfigSaveResult{}, err
		}
...
func (s *Service) TestConfig(ctx context.Context, stepKey string, input Input, values map[string]any) (TestResult, error) {
	...
	if !def.Testable {
		...
		return TestResult{StepKey: stepKey, Status: "skipped", SummaryKey: "ui.setup.tests.skipped", InputFingerprint: inputFingerprint, TestedAt: time.Now().UTC()}, nil
	}
	...
	test.InputFingerprint = inputFingerprint
	if s.store != nil {
		if err := s.store.ensure(ctx); err != nil {
			return TestResult{}, err
		}
...
func (s *Service) SkipStep(ctx context.Context, runKey string, stepKey string, reason string, input Input) (RunResult, error) {
	...
	return s.result(ctx, &execution{service: s, input: input, run: run})
}
...
func (s *Service) runStep(ctx context.Context, exec *execution, def stepDefinition) error {
	if s.store != nil && exec.run != nil {
		...
		if err := s.store.saveRun(ctx, exec.run); err != nil {
			return err
		}
	}
	if s.store != nil {
	...
	failStep := func(summary string, stepErr error) error {
		if s.store != nil {
			_, err := s.store.upsertStep(ctx, exec.run, def, StepStatusFailed, 0, summary, stepErr)
			return err
		}
		return nil
	}
	...
	if def.Check != nil {
		...
		if err != nil {
			if recordErr := failStep(summary, err); recordErr != nil {
				return errors.Join(err, recordErr)
			}
			return err
		}
		if status == StepStatusSucceeded || status == StepStatusSkipped {
			if err := finishStep(status, summary); err != nil {
				return err
			}
			return s.printf("%s: %s\n", def.Title, summary)
		}
	...
	if err != nil {
		if recordErr := failStep(outcome.Summary, err); recordErr != nil {
			return errors.Join(err, recordErr)
		}
		return err
	}
	if err := finishStep(status, outcome.Summary); err != nil {
		return err
	}
	return s.printf("%s: %s\n", def.Title, outcome.Summary)
}

func (s *Service) result(ctx context.Context, exec *execution) (RunResult, error) {
	var steps []StepReport
	if s.store != nil && exec.run != nil {
		records, err := s.store.steps(ctx, exec.run.ID)
		if err != nil {
			return RunResult{}, err
		}
	...
	return []stepDefinition{
		{
			...
			Recovery:         "可继续使用 Web 向导，或在 CLI 中运行 console init。",
			Required:         true,
			Retryable:        true,
			Idempotent:       true,
		...
		},
		{
			...
			Recovery:         "检查 --config 参数或 CONFIG_PATH 环境变量。",
			Required:         true,
			Retryable:        true,
			Idempotent:       true,
		...
		},
		{
			...
			Order:            30,
			Title:            "配置诊断",
			Goal:             "集中检查服务、数据库、认证、安全、WebUI 和存储配置。",
			AutomaticActions: []string{"执行配置诊断", "返回阻塞项和环境变量候选名"},
			CompletionMark:   "无阻塞配置诊断",
			Recovery:         "运行 console init 或 console run 根据提示修复配置。",
			Required:         true,
			Retryable:        true,
			Idempotent:       true,
		...
		},
		{
			...
			AutomaticActions: []string{"Ping 数据库", "检查 Redis、存储和 WebUI 启用状态"},
...
func (s *Service) pendingRestart() (bootstrapState, bool, error) {
...
func (s *Service) printf(format string, args ...any) error {
	...
	if _, err := fmt.Fprintf(s.stdout, format, args...); err != nil {
		return fmt.Errorf("write initialization output: %w", err)
	}
	return nil
}
...
func setupTokenValid(value string) bool {
	...
	for _, key := range []string{"CONSOLE_SETUP_TOKEN", "APP_SETUP_TOKEN"} {
