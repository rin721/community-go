...
func Start(ctx context.Context, transport initapp.Transport) error {
	...
	if err := transport.HTTPServer.Start(ctx); err != nil {
		return errors.Join(
			fmt.Errorf("start HTTP server: %w", err),
	...
	if transport.RPCServer != nil {
		if err := transport.RPCServer.Start(ctx); err != nil {
			...
			if shutdownErr := transport.HTTPServer.Shutdown(shutdownCtx); shutdownErr != nil {
				rollbackErr = errors.Join(rollbackErr, fmt.Errorf("http server rollback shutdown: %w", shutdownErr))
			}
			rollbackErr = errors.Join(rollbackErr, shutdownBackground(shutdownCtx, transport.Background))
			return errors.Join(fmt.Errorf("start RPC server: %w", err), rollbackErr)
		}
	}
	return nil
}
...
//
// Start 保持非阻塞启动语义；命令行主进程使用 Run，让异步 Serve 错误能够回到 cmd/console。
...
func Shutdown(ctx context.Context, core initapp.Core, infra initapp.Infrastructure, transport initapp.Transport) error {
	...
	if len(errs) > 0 {
		return errors.Join(append([]error{fmt.Errorf("shutdown completed with %d errors", len(errs))}, errs...)...)
	}
	return nil
}
...
func startBackground(ctx context.Context, services []initapp.BackgroundService) error {
	for i, service := range services {
		...
		if err := service.Start(ctx); err != nil {
			return errors.Join(
				fmt.Errorf("start background service: %w", err),
...
//
// 该 helper 只用于启动失败回滚，错误会返回给启动调用方；正式关闭路径由 Shutdown 收集并记录错误。
func shutdownBackground(ctx context.Context, services []initapp.BackgroundService) error {
	var joined error
	for i := len(services) - 1; i >= 0; i-- {
		if services[i] != nil {
			if err := services[i].Shutdown(ctx); err != nil {
				joined = errors.Join(joined, fmt.Errorf("background rollback shutdown: %w", err))
			}
		}
	}
	return joined
