...
func reloadCache(core *initapp.Core, infra *initapp.Infrastructure, cfg *config.Config) {
	if cfg.Cache.Driver == config.CacheDriverDisabled {
		if infra.Cache != nil {
			if err := infra.Cache.Close(); err != nil {
				warnCloseFailure(core, "cache", "disable", err)
			}
			infra.Cache = nil
		}
	...
	if old != nil {
		if err := old.Close(); err != nil {
			warnCloseFailure(core, "cache", "replace", err)
		}
	}
	core.Logger.Info("cache reloaded", "driver", cfg.Cache.Driver)
}
	...
	if !cfg.Executor.Enabled {
		if infra.Executor != nil {
			if err := infra.Executor.Shutdown(); err != nil {
				core.Logger.Error("failed to shutdown executor", "error", err)
			}
			infra.Executor = nil
		}
	...
	if cfg.Storage.Driver == config.StorageDriverDisabled || cfg.Storage.Driver == config.StorageDriverS3 || cfg.Storage.Driver == config.StorageDriverMinIO {
		if infra.Storage != nil {
			if err := infra.Storage.Close(); err != nil {
				warnCloseFailure(core, "storage", "disable", err)
			}
			infra.Storage = nil
		}
...
// warnCloseFailure 只记录热重载清理失败，不改变 reload 的 best-effort 语义。
func warnCloseFailure(core *initapp.Core, component string, action string, err error) {
	if err == nil || core == nil || core.Logger == nil {
		return
	}
	core.Logger.Warn(component+" close failed", "action", action, "error", err)
}
