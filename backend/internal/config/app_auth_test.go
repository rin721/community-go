...
func TestAuthConfigValidateRejectsNonPositiveCasbinReloadInterval(t *testing.T) {
	cfg := validSMTPAuthConfig()
	cfg.CasbinReloadIntervalSeconds = -1

	err := cfg.Validate()
	if err == nil || !strings.Contains(err.Error(), "casbin_reload_interval_seconds must be positive") {
		t.Fatalf("Validate() error = %v, want invalid casbin reload interval", err)
	}
}

func TestAuthConfigValidateRejectsNonPositiveNotificationRetry(t *testing.T) {
	cfg := validSMTPAuthConfig()
	cfg.NotificationRetryIntervalSeconds = -1

	err := cfg.Validate()
	if err == nil || !strings.Contains(err.Error(), "notification retry interval") {
		t.Fatalf("Validate() error = %v, want invalid notification retry policy", err)
	}
}
