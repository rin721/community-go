...
func (c *Config) diagnoseAuth(add func(string, string, string)) {
	...
	if c.Auth.CasbinReloadIntervalSeconds <= 0 {
		add(AppAuthName, "auth.casbin_reload_interval_seconds", "casbin_reload_interval_seconds must be positive")
	}
	if c.Auth.NotificationRetryIntervalSeconds <= 0 {
		add(AppAuthName, "auth.notification_retry_interval_seconds", "notification_retry_interval_seconds must be positive")
	}
	if c.Auth.NotificationRetryBatchSize <= 0 {
		add(AppAuthName, "auth.notification_retry_batch_size", "notification_retry_batch_size must be positive")
	}
	if c.Auth.NotificationRetryMaxAttempts <= 0 {
		add(AppAuthName, "auth.notification_retry_max_attempts", "notification_retry_max_attempts must be positive")
	}
