...
SELECT
    MIN(id),
    COALESCE(NULLIF(product_code, ''), 'console-platform') AS normalized_product_code,
    'tenant',
    code,
    MIN(name),
    MIN(description),
...
GROUP BY COALESCE(NULLIF(product_code, ''), 'console-platform'), code;
