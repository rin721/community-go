...
type NotificationOutboxFilter = iamservice.NotificationOutboxFilter
...
func (r *repository) ListUserRoleCodes(ctx context.Context, orgID, userID int64) ([]string, error) {
	var rows []model.CasbinRule
	if err := r.db.Find(ctx, &rows,
		database.Where("ptype = ? AND v0 = ? AND v2 = ?", "g", "user:"+strconv.FormatInt(userID, 10), strconv.FormatInt(orgID, 10)),
		database.Order("v1 ASC"),
	); err != nil {
		return nil, err
	}
	roles := make([]string, 0, len(rows))
	for _, row := range rows {
		roles = append(roles, strings.TrimPrefix(row.V1, "role:"))
	}
	return roles, nil
}
...
func (r *repository) FindPasswordResetByID(ctx context.Context, id int64) (*model.PasswordReset, error) {
	var reset model.PasswordReset
	if err := r.db.First(ctx, &reset, database.Where("id = ?", id)); err != nil {
		return nil, err
	}
	return &reset, nil
}
...
func (r *repository) FindEmailVerificationByID(ctx context.Context, id int64) (*model.EmailVerification, error) {
	var verification model.EmailVerification
	if err := r.db.First(ctx, &verification, database.Where("id = ?", id)); err != nil {
		return nil, err
	}
...
func (r *repository) CreateNotificationOutbox(ctx context.Context, outbox *model.NotificationOutbox) error {
	return r.db.Create(ctx, outbox)
}

func (r *repository) FindNotificationOutboxByID(ctx context.Context, id int64) (*model.NotificationOutbox, error) {
	var outbox model.NotificationOutbox
	if err := r.db.First(ctx, &outbox, database.Where("id = ?", id)); err != nil {
		return nil, err
	}
	return &outbox, nil
}

func (r *repository) ListDueNotificationOutbox(ctx context.Context, now time.Time, limit int) ([]model.NotificationOutbox, error) {
	if limit <= 0 || limit > 100 {
		limit = 20
	}
	var items []model.NotificationOutbox
	err := r.db.Find(ctx, &items,
		database.Where("status = ?", model.NotificationOutboxStatusPending),
		database.Where("next_attempt_at <= ?", now),
		database.Where("attempts < max_attempts"),
		database.Order("next_attempt_at ASC, id ASC"),
		database.Limit(limit),
	)
	return items, err
}

func (r *repository) ListNotificationOutbox(ctx context.Context, filter NotificationOutboxFilter) ([]model.NotificationOutbox, int64, error) {
	if filter.Page <= 0 {
		filter.Page = 1
	}
	if filter.PageSize <= 0 || filter.PageSize > 100 {
	...
	if filter.Kind != "" {
		opts = append(opts, database.Where("kind = ?", filter.Kind))
	}
	if recipient := strings.ToLower(strings.TrimSpace(filter.Recipient)); recipient != "" {
		opts = append(opts, database.Where("LOWER(recipient) LIKE ?", "%"+recipient+"%"))
	}
	total, err := r.db.Count(ctx, &model.NotificationOutbox{}, opts...)
	if err != nil {
		return nil, 0, err
	}
	query := append(append([]database.QueryOption{}, opts...),
...
func (r *repository) SaveNotificationOutbox(ctx context.Context, outbox *model.NotificationOutbox) error {
	outbox.UpdatedAt = time.Now().UTC()
	return r.db.Save(ctx, outbox)
}
