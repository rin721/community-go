...
func (s *service) RunTrafficProbe(ctx context.Context, id int64) (model.TrafficProbeResult, error) {
	...
	if result.EvidenceJSON, err = normalizeTrafficEvidenceJSON(result.EvidenceJSON); err != nil {
		return model.TrafficProbeResult{}, err
	}
	...
	if err := s.repo.DeleteOldTrafficProbeResults(ctx, target.ID, defaultTrafficProbeResultKeep); err != nil {
		s.warnTrafficProbeCleanupError("traffic probe old result cleanup failed", target.ID, err)
	}
...
func (s *service) warnTrafficProbeCleanupError(message string, targetID int64, err error) {
	if s.logger == nil || err == nil {
		return
	}
	s.logger.Warn(message, "target_id", targetID, "error", err)
}
...
func normalizeTrafficEvidenceJSON(value string) (string, error) {
	...
	if err := json.Unmarshal([]byte(value), &raw); err != nil {
		return "", fmt.Errorf("%w: invalid traffic probe evidence json: %w", ErrInvalidInput, err)
	}
