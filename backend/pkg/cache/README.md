...
- 当前风险：[CONFIRMED] Redis 依赖路径已有隔离行为测试；使用方仍应通过接口注入而不是依赖具体实现。
...
- ✅ **Hybrid 模式** - 初始化 Redis 不可用时可退成本地缓存；运行中 Redis 错误会返回给调用方处理
- ✅ **原子化重载** - 支持配置热更新，不中断服务
- ✅ **线程安全** - 所有操作都是并发安全的
- ✅ **连接池** - 高效的连接池管理
...
`Reload` 会先验证新 Redis 连接，验证失败时保持旧连接不变；如果关闭失败候选连接也失败，会用 `errors.Join` 同时返回验证错误和候选关闭错误。新连接验证通过后会先原子替换，再关闭旧连接；此时如果旧连接关闭失败，返回错误会明确说明 reload 已经应用但旧资源关闭异常，调用方应记录并按运行态策略告警或补偿。
...
`hybrid` 模式只在创建实例时允许 Redis 不可用并退化为本地缓存；如果 Redis 创建成功后运行中失败，`Exists`、`MGet`、`IncrBy` 等方法会返回 Redis 错误。调用方可以结合已返回的本地部分结果自行降级、重试或写日志，不应把远端错误误判成“数据不存在”。

当 Redis 读取或计数成功后，hybrid 会尝试把结果回填到本地缓存。该回填失败不会丢弃已经拿到的 Redis 值，`Get`、`MGet`、`IncrBy` 会同时返回可用结果和带 key 上下文的本地回填错误，由调用方决定是否继续使用结果、记录告警或触发降级。

Redis 创建、热重载和关闭路径不会吞掉连接关闭错误：创建或重载候选连接验证失败时，候选连接关闭失败会与主错误一起返回；重载已经切到新连接后，如果旧连接关闭失败，也会把“已应用但旧连接关闭失败”的状态返回给上层。
