...
func (h *hybridCache) Get(ctx context.Context, key string) (string, error) {
	...
	if err := h.local.Set(ctx, key, value, 0); err != nil {
		return value, fmt.Errorf("hybrid local backfill set %s: %w", key, err)
	}
	return value, nil
}
...
func (h *hybridCache) MGet(ctx context.Context, keys ...string) ([]interface{}, error) {
	...
	if h.redis != nil && len(missing) > 0 {
		...
		for i, value := range redisValues {
			...
			if err := h.local.Set(ctx, missing[i], text, 0); err != nil {
				backfillErr = errors.Join(backfillErr, fmt.Errorf("hybrid local backfill set %s: %w", missing[i], err))
			}
		}
		if backfillErr != nil {
			return values, backfillErr
		}
	}
	return values, nil
}
...
func (h *hybridCache) IncrBy(ctx context.Context, key string, value int64) (int64, error) {
	...
	if h.redis != nil {
		redisNext, redisErr := h.redis.IncrBy(ctx, key, value)
		if redisErr != nil {
			return next, redisErr
		}
		next = redisNext
		if err := h.local.Set(ctx, key, next, 0); err != nil {
			return next, fmt.Errorf("hybrid local counter sync %s: %w", key, err)
		}
	}
	return next, nil
}
