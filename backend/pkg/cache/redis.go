...
func NewRedis(config *Config, logger Logger) (Cache, error) {
	...
	if err := client.Ping(ctx).Err(); err != nil {
		closeErr := closeRedisClient(client, "redis connection candidate close failed")
		if logger != nil {
			logger.Error(MsgCacheConnectionFailed, "error", err)
		}
		return nil, errors.Join(fmt.Errorf(ErrMsgConnectionFailed, err), closeErr)
	}
...
func (r *redisCache) Reload(ctx context.Context, newConfig *Config) error {
	...
	if err := newClient.Ping(ctx).Err(); err != nil {
		closeErr := closeRedisClient(newClient, "redis reload candidate close failed")
		if r.logger != nil {
			r.logger.Error(MsgCacheReloadFailed, "error", err)
		}
		return errors.Join(fmt.Errorf(ErrMsgReloadFailed, err), closeErr)
	}
	...
	// 在锁外执行,避免阻塞
	if oldClient != nil {
		if err := closeRedisClient(oldClient, "redis reload applied but old client close failed"); err != nil {
			if r.logger != nil {
				r.logger.Error("failed to close old redis client after reload", "error", err)
			}
			return err
		}
	}
...
func closeRedisClient(client redisCloser, context string) error {
	if client == nil {
		return nil
	}
