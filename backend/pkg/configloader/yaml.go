...
func writeFilePreservingIdentity(path string, oldContent, newContent []byte, mode os.FileMode) (err error) {
	...
	defer func() {
		if tempPath != "" {
			err = joinCleanupError(err, removeConfigTempFile(tempPath))
		}
	}()

	if err := tempFile.Chmod(mode); err != nil {
		return closeConfigFileAfterError(err, tempFile, "close config temp file after chmod failure")
	}
	if _, err := tempFile.Write(newContent); err != nil {
		return closeConfigFileAfterError(err, tempFile, "close config temp file after write failure")
	}
	if err := tempFile.Sync(); err != nil {
		return closeConfigFileAfterError(err, tempFile, "close config temp file after sync failure")
	}
	...
	if _, err := targetFile.Write(newContent); err != nil {
		return restoreFileContent(path, oldContent, mode, closeConfigFileAfterError(err, targetFile, "close config target file after write failure"))
	}
	if err := targetFile.Sync(); err != nil {
		return restoreFileContent(path, oldContent, mode, closeConfigFileAfterError(err, targetFile, "close config target file after sync failure"))
	}
	if err := targetFile.Close(); err != nil {
		return restoreFileContent(path, oldContent, mode, err)
	}
	cleanupPath := tempPath
	tempPath = ""
	if err := removeConfigTempFile(cleanupPath); err != nil {
		return err
	}
	return nil
}

func closeConfigFileAfterError(cause error, file interface{ Close() error }, context string) error {
	if cause == nil {
		return nil
	}
	if err := file.Close(); err != nil {
		return errors.Join(cause, fmt.Errorf("%s: %w", context, err))
	}
	return cause
}

func removeConfigTempFile(path string) error {
	if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("remove config temp file: %w", err)
	}
	return nil
}

func joinCleanupError(primary error, cleanup error) error {
	if cleanup == nil {
		return primary
	}
	if primary == nil {
...
// restoreFileContent 在写入失败后尽力恢复旧配置内容，并把恢复失败一并返回给调用方。
func restoreFileContent(path string, oldContent []byte, mode os.FileMode, cause error) error {
	return restoreFileContentWithWriter(path, oldContent, mode, cause, os.WriteFile)
}

func restoreFileContentWithWriter(path string, oldContent []byte, mode os.FileMode, cause error, writer func(string, []byte, os.FileMode) error) error {
	if cause == nil {
		return nil
	}
	if err := writer(path, oldContent, mode); err != nil {
		return errors.Join(cause, fmt.Errorf("restore config file after failed write: %w", err))
	}
	return cause
}
