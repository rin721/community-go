...
func (d *database) Reload(cfg *Config) error {
	...
	if err := newDB.Ping(context.Background()); err != nil {
		// 新连接不可用,保持旧连接不变,并把候选连接关闭失败也返回给上层
		return errors.Join(
			fmt.Errorf("new database connection ping failed: %w", err),
			closeReloadCandidate(newDB),
		)
	}
...
func closeReloadCandidate(db Database) error {
	if db == nil {
		return nil
	}
	if err := db.Close(); err != nil {
		return fmt.Errorf("database reload candidate close failed: %w", err)
	}
	return nil
}
