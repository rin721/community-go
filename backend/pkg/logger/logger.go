...
type Logger interface {
	...
	//   error: 刷新失败时的错误
	// 使用场景:
	//   defer func() { if err := log.Sync(); err != nil { report(err) } }()
	//   if err := log.Sync(); err != nil { return err } // 在优雅关闭或 CLI 中返回给调用方
	// 为什么需要:
	//   日志可能被缓冲以提高性能
	//   如果程序突然退出,缓冲的日志可能丢失
	//   Sync 确保所有日志都被写入
	//   调用方应处理返回错误；日志记录不能替代错误返回
	Sync() error
...
type Reloader interface {
	...
	//   cfg: 新的日志配置
	// 返回:
	//   error: 新配置创建失败或旧 logger 同步失败时的错误
	// 并发安全:
	//   - 使用读写锁保护,确保重载过程原子性
	//   - 新 logger 创建失败时保持原有 logger 不变
	//   - 新 logger 创建成功后才替换旧 logger
	//   - 替换后旧 logger 同步失败会返回“重载已应用但旧 logger 同步失败”的错误状态
