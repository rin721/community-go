...
func TestNormalizeSyncErrorIgnoresPlatformInvalidArgument(t *testing.T) {
	err := normalizeSyncError(fmt.Errorf("sync stdout: %w", syscall.EINVAL))
	if err != nil {
		t.Fatalf("normalizeSyncError(EINVAL) = %v, want nil", err)
	}
}

func TestNormalizeSyncErrorKeepsRealError(t *testing.T) {
	syncErr := errors.New("disk sync failed")
	err := normalizeSyncError(syncErr)
	if !errors.Is(err, syncErr) {
		t.Fatalf("normalizeSyncError() = %v, want %v", err, syncErr)
	}
}

func TestReloadReturnsOldLoggerSyncError(t *testing.T) {
	syncErr := errors.New("old logger sync failed")
	log := newTestZapLoggerWithSyncError(syncErr)

	err := log.Reload(&Config{
		Level:  "debug",
		Format: "console",
		Output: "stdout",
	})
	...
	if !errors.Is(err, syncErr) {
		t.Fatalf("Reload() error = %v, want old sync error", err)
	}
	if log.config == nil || log.config.Level != "debug" {
		t.Fatalf("Reload() should apply new config before returning old sync error, got %#v", log.config)
	}
}

func TestReloadIgnoresOldLoggerInvalidArgumentSyncError(t *testing.T) {
	log := newTestZapLoggerWithSyncError(fmt.Errorf("sync stdout: %w", syscall.EINVAL))

	if err := log.Reload(&Config{
		Level:  "debug",
		Format: "console",
		Output: "stdout",
	}); err != nil {
		t.Fatalf("Reload() error = %v, want nil for benign stdout sync", err)
	}
}

func newTestZapLoggerWithSyncError(syncErr error) *zapLogger {
	core := zapcore.NewCore(
		zapcore.NewConsoleEncoder(zap.NewDevelopmentEncoderConfig()),
		syncErrorWriteSyncer{err: syncErr},
		zapcore.DebugLevel,
	)
	return &zapLogger{
...
func (s syncErrorWriteSyncer) Write(p []byte) (int, error) {
	return len(p), nil
}

func (s syncErrorWriteSyncer) Sync() error {
	return s.err
}
