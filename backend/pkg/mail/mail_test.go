...
func TestBuildMessageEncodesHeadersAndMultipart(t *testing.T) {
	from := netmail.Address{Name: "Console Platform", Address: "noreply@example.com"}
	...
	for _, want := range []string{
		"From: \"Console Platform\" <noreply@example.com>",
...
func TestTemplateRender(t *testing.T) {
	...
	}).Render(map[string]any{"ProductName": "Console Platform", "URL": "https://example.com"})
	if err != nil {
		t.Fatalf("Render() failed: %v", err)
	}
	if rendered.Subject != "Welcome Console Platform" || rendered.TextBody != "Open https://example.com" {
...
func TestSMTPSenderSend(t *testing.T) {
	...
	sender, err := NewSMTP(Config{Host: "localhost", Port: server.port, From: "noreply@example.com", FromName: "Console Platform"})
	if err != nil {
		t.Fatalf("NewSMTP() failed: %v", err)
	}
...
func TestSMTPSenderSendReturnsDataCloseError(t *testing.T) {
	server := startFakeSMTP(t, fakeSMTPOptions{dataCloseResponse: "451 data rejected"})
	sender, err := NewSMTP(Config{Host: "localhost", Port: server.port, From: "noreply@example.com"})
	if err != nil {
		t.Fatalf("NewSMTP() failed: %v", err)
	}
	...
	if !strings.Contains(err.Error(), "451") || !strings.Contains(err.Error(), "data rejected") {
		t.Fatalf("Send() error = %v, want SMTP DATA rejection", err)
	}
}

func TestCloseSMTPDataWriterReturnsCloseError(t *testing.T) {
	closeErr := errors.New("writer close failed")
	err := closeSMTPDataWriter(closeErrorCloser{err: closeErr})
	if err == nil {
		t.Fatal("closeSMTPDataWriter() error = nil, want close error")
	}
	if !errors.Is(err, closeErr) {
		t.Fatalf("closeSMTPDataWriter() error = %v, want %v", err, closeErr)
	}
	if !strings.Contains(err.Error(), "smtp data writer close") {
		t.Fatalf("closeSMTPDataWriter() error = %v, want close context", err)
	}
}

func TestApplySMTPDeadlineReturnsSetDeadlineError(t *testing.T) {
	deadlineErr := errors.New("deadline unsupported")
	ctx, cancel := context.WithDeadline(context.Background(), time.Now().Add(time.Minute))
	defer cancel()

	err := applySMTPDeadline(ctx, deadlineErrorConn{err: deadlineErr}, time.Second)

	if !errors.Is(err, deadlineErr) {
		t.Fatalf("applySMTPDeadline() error = %v, want %v", err, deadlineErr)
	}
	if !strings.Contains(err.Error(), "smtp connection deadline") {
		t.Fatalf("applySMTPDeadline() error missing context: %v", err)
	}
}

func TestApplySMTPDeadlineSkipsWhenNoDeadlineOrTimeout(t *testing.T) {
	err := applySMTPDeadline(context.Background(), deadlineErrorConn{err: errors.New("should not be used")}, 0)

	if err != nil {
		t.Fatalf("applySMTPDeadline() error = %v, want nil", err)
	}
}
	...
	for {
		...
		if inData {
			if line == "." {
				...
				data.Reset()
				inData = false
				if s.options.dataCloseResponse != "" {
					writeSMTPLine(writer, s.options.dataCloseResponse)
				} else {
...
func (c deadlineErrorConn) Read([]byte) (int, error) {
	return 0, nil
}

func (c deadlineErrorConn) Write([]byte) (int, error) {
	return 0, nil
}
...
func (c deadlineErrorConn) LocalAddr() net.Addr {
	return nil
}

func (c deadlineErrorConn) RemoteAddr() net.Addr {
	return nil
}
