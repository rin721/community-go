...
```bash
go get github.com/open-console/console-platform/pkg/storage
```
...
监听启动失败会由 `Watch` 直接返回错误；监听启动后的异步后端错误会以 `WatchEvent{Op: "ERROR", Error: err}` 传给 handler，调用方应自行决定记录、重试或告警，不要只按事件类型忽略错误原因。
...
`StorageManager.Close` 会继续尝试关闭本地与对象存储 client；如果多个关闭动作失败，会使用 `errors.Join` 同时返回所有底层错误。`local+s3` 与 `local+minio` 组合驱动在对象存储初始化失败时，也会关闭已经创建的本地 client，并把初始化错误与本地关闭错误一并返回给调用方。
