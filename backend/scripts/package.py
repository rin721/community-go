...
DEFAULT_BINARY_NAME = "console-server"
WEBUI_DIST = Path("web/app/build/client")
CGO_DISABLED_SQLITE_NOTE = (
    "SQLite runtime: unavailable with CGO_ENABLED=0; use MySQL/Postgres, "
    "or rebuild on the target platform with --cgo when SQLite is required."
)
...
def app_metadata(root: Path, version_override: str | None) -> tuple[str, str]:
    name = os.environ.get("APP_PACKAGE_NAME", DEFAULT_BINARY_NAME).strip() or DEFAULT_BINARY_NAME
    version = version_override or os.environ.get("APP_VERSION", "").strip() or git_commit(root)
    return name, version
...
def run_command(root: Path, command: Sequence[str], *, cwd: Path | None = None, env: dict[str, str] | None = None, dry_run: bool = False, verbose: bool = False) -> None:
    ...
    if env:
        next_env.update(env)
    subprocess.run(resolve_command(command, next_env), cwd=display_cwd, env=next_env, check=True)


def resolve_command(command: Sequence[str], env: dict[str, str]) -> list[str]:
    if not command:
    ...
    if resolved is None and os.name == "nt":
        resolved = shutil.which(f"{executable}.cmd", path=env.get("PATH")) or shutil.which(f"{executable}.exe", path=env.get("PATH"))
    if resolved is None:
...
def build_binary(root: Path, staging: Path, target: Target, opts: Options) -> Path:
    ...
    run_command(
        root,
        ["go", "build", "-mod=readonly", "-trimpath", "-ldflags=-s -w", "-o", str(binary), "./cmd/console"],
        env={
            "GOOS": target.goos,
...
def write_release_readme(package_root: Path, include_webui: bool, opts: Options) -> None:
    lines = [
        ...
        f"SQLite runtime: {'available' if opts.cgo else 'unavailable'}.",
    ]
    if not opts.cgo:
        lines.append(CGO_DISABLED_SQLITE_NOTE)
    lines.extend(
        [
...
def plan_only(root: Path, output: Path, opts: Options, app_name: str, version: str, include_webui: bool) -> None:
    ...
    print(f"SQLite runtime: {'available' if opts.cgo else 'unavailable'}")
    if not opts.cgo:
        print(CGO_DISABLED_SQLITE_NOTE)
...
def main(argv: Sequence[str]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="console-package-") as temp_dir:
        ...
        for target in opts.targets:
            ...
            manifest = {
                ...
                "cgoEnabled": opts.cgo,
                "sqliteRuntimeAvailable": opts.cgo,
