...
func TestSuccessErrorAndPaginationContracts(t *testing.T) {
	...
	invalidPageSize := NewPageResult([]string{"a"}, 1, 0, 1)
	if invalidPageSize.Pagination.TotalPages != 0 {
		t.Fatalf("invalid pageSize TotalPages = %d, want 0", invalidPageSize.Pagination.TotalPages)
	}
	if invalidPageSize.Pagination.PageSize != 0 || invalidPageSize.Pagination.Total != 1 {
		t.Fatalf("invalid pageSize metadata should preserve caller context: %+v", invalidPageSize.Pagination)
	}
}
