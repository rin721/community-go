    ...
    for (const field of step.fields) {
      if (field.key === "i18n.defaultLocale") {
        if (
          ...
          scalarString(bucket[field.key]) === legacyLocale(previousLocale)
        ) {
          bucket[field.key] = locale;
        }
        continue;
      }
...
export function syncDefaultLocaleValue(
  ...
  for (const step of schema.steps.map(normalizeStep)) {
    ...
    const bucket = { ...(next[step.key] ?? {}) };
    if (bucket["i18n.defaultLocale"] !== locale) {
      bucket["i18n.defaultLocale"] = locale;
      changed = true;
    }
    next[step.key] = bucket;
  }
...
function legacyLocale(locale: AppLocale | undefined) {
