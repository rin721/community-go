...
describe("API endpoint registry", () => {
  it("matches the current Go health, readiness, setup, and auth route paths", () => {
  ...
  it("matches the current Go IAM organization route paths", () => {
    ...
    expect(API_ENDPOINTS.iam.notificationOutbox).toBe("/api/v1/iam/notification-outbox");
    expect(API_ENDPOINTS.iam.notificationOutboxRetry("outbox/7")).toBe(
      "/api/v1/iam/notification-outbox/outbox%2F7/retry",
    );
  });
