...
const dictionaryErrorCopy = {
  defaultTitle: "admin.dictionaries.states.errorTitle",
  permissionDescription: "admin.dictionaries.states.permissionDescription",
  permissionTitle: "admin.dictionaries.states.permissionTitle",
  storageUnavailableDescription: "admin.dictionaries.states.storageUnavailableDescription",
  storageUnavailableTitle: "admin.dictionaries.states.storageUnavailableTitle",
};
...
export default function AdminDictionariesRoute() {
  ...
  const canCreateDictionary = hasSessionPermission(permissions, {
    code: "dictionary:create",
    productCode: productCode || undefined,
    scope: "platform",
  });
  const canUpdateDictionary = hasSessionPermission(permissions, {
    code: "dictionary:update",
    productCode: productCode || undefined,
    scope: "platform",
  });
  const canDeleteDictionary = hasSessionPermission(permissions, {
  ...
  const createDictionaryMutation = useMutation({
    ...
    onError: (error) => {
      setNotice({
        description: mutationErrorDescription(error, "dictionary:create", t),
        intent: "danger",
  ...
  const updateDictionaryMutation = useMutation({
    ...
    onError: (error) => {
      setNotice({
        description: mutationErrorDescription(error, "dictionary:update", t),
        intent: "danger",
        title: t("admin.dictionaries.messages.saveFailedTitle"),
      });
    },
  ...
  const deleteDictionaryMutation = useMutation({
    ...
    onError: (error) => {
      setNotice({
        description: mutationErrorDescription(error, "dictionary:delete", t),
        intent: "danger",
  ...
  const dictionaryFormWritable =
    dictionaryForm?.mode === "create" ? canCreateDictionary : canUpdateDictionary;
  const itemFormWritable = canUpdateDictionary;
  ...
  const startCreateDictionary = () => {
    if (!canCreateDictionary || !storagePersisted || writePending) {
      return;
    }
  ...
  const startEditDictionary = useCallback(
    (dictionary: SystemDictionary) => {
      if (!canUpdateDictionary || !storagePersisted || writePending) {
        return;
      }
    ...
    [canUpdateDictionary, closeItemForm, storagePersisted, writePending],
  );
  ...
  const startCreateItem = useCallback(
    ...
    [canUpdateDictionary, closeDictionaryForm, storagePersisted, writePending],
  );
  ...
  const startDeleteDictionary = useCallback(
    (dictionary: SystemDictionary) => {
      if (!canDeleteDictionary || !storagePersisted || writePending) {
        return;
      }
      setPendingDelete({ dictionary, mode: "dictionary" });
    },
    [canDeleteDictionary, storagePersisted, writePending],
  );

  const startDeleteItem = useCallback(
    (dictionary: SystemDictionary, item: SystemDictionaryItem) => {
      if (!canDeleteDictionary || !storagePersisted || writePending) {
        return;
      }
      setPendingDelete({ dictionary, item, mode: "item" });
    },
    [canDeleteDictionary, storagePersisted, writePending],
  );
    ...
    (dictionary: SystemDictionary): ColumnDef<SystemDictionaryItem>[] => [
      {
        accessorKey: "label",
        cell: ({ row }) => (
          <div className="console-dictionary-item-name">
      ...
      },
      {
        accessorKey: "value",
        cell: ({ getValue }) => (
          <code className="console-dictionary-code">{String(getValue())}</code>
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return value ? (
            <span className="console-dictionary-extra">{value}</span>
          ) : (
      ...
      },
      {
        id: "actions",
        cell: ({ row }) => (
          <div className="console-dictionary-item-actions">
            <Button
              appearance="secondary"
              aria-label={t("admin.dictionaries.actions.editItemFor", {
                label: row.original.label,
              })}
              disabled={!canUpdateDictionary || !storagePersisted || writePending}
              icon={<Pencil size={15} />}
              onClick={() => startEditItem(dictionary, row.original)}
              title={
                canUpdateDictionary ? undefined : dictionaryPermissionTitle("dictionary:update", t)
              }
            >
            ...
            <Button
              ...
              disabled={!canDeleteDictionary || !storagePersisted || writePending}
              icon={<Trash2 size={15} />}
              onClick={() => startDeleteItem(dictionary, row.original)}
              title={
                canDeleteDictionary ? undefined : dictionaryPermissionTitle("dictionary:delete", t)
              }
            >
  ...
  const submitDictionary = (event: FormEvent<HTMLFormElement>) => {
    ...
    const permission = dictionaryForm.mode === "create" ? "dictionary:create" : "dictionary:update";
    if (
      (dictionaryForm.mode === "create" && !canCreateDictionary) ||
      (dictionaryForm.mode === "edit" && !canUpdateDictionary)
    ) {
      setNotice({
        description: dictionaryPermissionTitle(permission, t),
        intent: "danger",
        title: t("admin.dictionaries.states.permissionTitle"),
      });
      return;
    }
    if (!storagePersisted) {
  ...
  const submitItem = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!canUpdateDictionary) {
      setNotice({
        description: dictionaryPermissionTitle("dictionary:update", t),
        intent: "danger",
        title: t("admin.dictionaries.states.permissionTitle"),
      });
      return;
  ...
  const confirmPendingDelete = () => {
    if (!pendingDelete || !storagePersisted || !canDeleteDictionary) {
      return;
    }
    setNotice(null);
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-dictionaries-title">
      <div className="console-admin-page-header">
        ...
        <div className="console-dictionary-page-actions">
          <Button
            disabled={!canCreateDictionary || !storagePersisted || writePending}
            icon={<Plus size={17} />}
            onClick={startCreateDictionary}
            title={
              canCreateDictionary ? undefined : dictionaryPermissionTitle("dictionary:create", t)
            }
          >
      ...
      {dictionariesQuery.error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(dictionariesQuery.error, t, dictionaryErrorCopy)}
          description={adminErrorDescription(dictionariesQuery.error, t, dictionaryErrorCopy)}
        />
      ) : null}
      ...
      {pendingDelete ? (
        <StateBlock
          action={
            <div className="console-dictionary-confirm-actions">
              <Button
                disabled={!canDeleteDictionary}
      ...
      {dictionaryForm ? (
        <section className="console-admin-panel console-dictionary-form-panel">
          ...
          <form className="console-dictionary-form-grid" onSubmit={submitDictionary}>
            <FormField
              disabled={dictionaryForm.mode === "edit" || !dictionaryFormWritable || writePending}
            ...
            />
            <FormField
              disabled={!dictionaryFormWritable || writePending}
            ...
            />
            <label className="console-form-field console-dictionary-form-field--span">
            ...
            <div className="console-dictionary-form-actions">
              <Button
                disabled={!dictionaryFormWritable || !dictionaryDraftValid || !storagePersisted}
                icon={<Save size={17} />}
      ...
      {itemForm ? (
        <section className="console-admin-panel console-dictionary-form-panel">
          ...
          <form className="console-dictionary-form-grid" onSubmit={submitItem}>
            <FormField
              disabled={!itemFormWritable || writePending}
            ...
            <div className="console-dictionary-form-actions">
              <Button
                disabled={!itemFormWritable || !itemDraftValid || !storagePersisted}
                icon={<Save size={17} />}
                loading={
      ...
      <div className="console-admin-stat-grid" aria-label={t("admin.dictionaries.summaryLabel")}>
        <DictionaryStatCard
      ...
      <section className="console-admin-panel">
        <header className="console-admin-panel-header-row">
          ...
          <span className="console-api-count">
        ...
        {dictionariesQuery.isLoading ? (
          ...
          <>
            ...
            {filteredDictionaries.length > 0 ? (
              <div className="console-dictionary-groups">
                {filteredDictionaries.map((dictionary) => (
                  <section className="console-dictionary-group" key={dictionary.id}>
                    <header>
                      <div className="console-dictionary-title">
                      ...
                      <div className="console-dictionary-actions">
                        <StatusPill
                        ...
                        <Button
                          ...
                          title={
                            canUpdateDictionary
                              ? undefined
                              : dictionaryPermissionTitle("dictionary:update", t)
                          }
                        >
                          {t("admin.dictionaries.actions.edit")}
                        </Button>
                        ...
                        <Button
                          ...
                          disabled={!canDeleteDictionary || !storagePersisted || writePending}
                          icon={<Trash2 size={15} />}
                          onClick={() => startDeleteDictionary(dictionary)}
                          title={
                            canDeleteDictionary
                              ? undefined
                              : dictionaryPermissionTitle("dictionary:delete", t)
                          }
                        >
                          {t("admin.dictionaries.actions.delete")}
                        </Button>
                    ...
                    {dictionary.description ? (
                      <p className="console-dictionary-description">{dictionary.description}</p>
                    ) : null}
                    <div className="console-dictionary-table">
                      <DataTable
...
function StatusPill({ label, status }: { label: string; status: string }) {
  return (
    <span className="console-dictionary-status" data-status={status || "unknown"}>
      {label}
    </span>
  );
}
...
function dictionaryPermissionTitle(permission: string, t: ReturnType<typeof useTranslation>["t"]) {
...
function mutationErrorDescription(
  ...
  return adminErrorDescription(error, t, dictionaryErrorCopy);
}
