...
const maxLimit = 200;
const loginLogErrorCopy = {
  defaultTitle: "admin.loginLogs.states.errorTitle",
  permissionDescription: "admin.loginLogs.states.permissionDescription",
  permissionTitle: "admin.loginLogs.states.permissionTitle",
};
...
export default function AdminLoginLogsRoute() {
  ...
  const loginLogColumns = useMemo<ColumnDef<IAMAuditLog>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.userId !== null && row.original.userId !== undefined ? (
            <code className="console-audit-code">{row.original.userId}</code>
          ) : (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="console-login-log-device">{deviceName(row.original.userAgent, t)}</span>
        ),
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-login-logs-title">
      ...
      {loginLogsQuery.error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(loginLogsQuery.error, t, loginLogErrorCopy)}
          description={adminErrorDescription(loginLogsQuery.error, t, loginLogErrorCopy)}
        />
      ) : null}

      <div className="console-admin-stat-grid" aria-label={t("admin.loginLogs.summaryLabel")}>
        <LoginLogStatCard
          icon={<LogIn size={19} />}
      ...
      <section className="console-admin-panel">
        ...
        {loginLogsQuery.isLoading ? (
          ...
          <div className="console-login-log-table">
            <DataTable
