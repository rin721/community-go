...
const menuErrorCopy = {
  defaultTitle: "admin.menus.states.errorTitle",
  permissionDescription: "admin.menus.states.permissionDescription",
  permissionTitle: "admin.menus.states.permissionTitle",
};
...
export default function AdminMenusRoute() {
  ...
  const filteredGroups = useMemo(() => filterMenuGroups(groups, filters, t), [filters, groups, t]);
    ...
    () => [
      {
        accessorKey: "label",
        cell: ({ row }) => (
          <div className="console-menu-name">
      ...
      {
        accessorKey: "path",
        cell: ({ getValue }) => <span className="console-menu-path">{String(getValue())}</span>,
        header: t("admin.menus.columns.path"),
      },
      {
        accessorKey: "permission",
        cell: ({ row }) => (
          <span className="console-menu-permission">
      ...
      },
      {
        accessorKey: "mobile",
        cell: ({ row }) => (
          <span className="console-menu-entry" data-mobile={String(row.original.mobile)}>
            {row.original.mobile
      ...
      {
        accessorKey: "icon",
        cell: ({ getValue }) => <span className="console-menu-code">{String(getValue())}</span>,
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-menus-title">
      ...
      {menusQuery.error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(menusQuery.error, t, menuErrorCopy)}
          description={adminErrorDescription(menusQuery.error, t, menuErrorCopy)}
        />
      ) : null}

      <div className="console-admin-stat-grid" aria-label={t("admin.menus.summaryLabel")}>
        <MenuStatCard
      ...
      <section className="console-admin-panel">
        ...
        {menusQuery.isLoading ? (
          ...
          filteredGroups.length > 0 ? (
            <div className="console-menu-groups">
              {filteredGroups.map((group) => (
                <section className="console-menu-group" key={group.code}>
                  <header>
                    <div>
                      <h3>{group.label}</h3>
                      <p>
                  ...
                  <div className="console-menu-table">
                    <DataTable
                      columns={columns}
                      data={group.items}
