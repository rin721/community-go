...
const operationRecordErrorCopy = {
  defaultTitle: "admin.operationRecords.states.errorTitle",
  permissionDescription: "admin.operationRecords.states.permissionDescription",
  permissionTitle: "admin.operationRecords.states.permissionTitle",
  storageUnavailableDescription: "admin.operationRecords.states.storageUnavailableDescription",
  storageUnavailableTitle: "admin.operationRecords.states.storageUnavailableTitle",
};
...
export default function AdminOperationRecordsRoute() {
  ...
  const deleteOperationRecordsMutation = useMutation({
    ...
    onError: (error) => {
      setNotice({
        description: deleteErrorDescription(error, t),
        intent: "danger",
  ...
  const canDeleteOperationRecords = hasSessionPermission(permissions, {
  ...
  const toggleRecordSelection = useCallback(
    (record: SystemOperationRecord, checked: boolean) => {
      if (!canDeleteOperationRecords || !storagePersisted || writePending) {
        return;
      }
    ...
    [canDeleteOperationRecords, storagePersisted, writePending],
  );
  ...
  const toggleAllVisible = useCallback(
    ...
    [canDeleteOperationRecords, operationRecordItems, storagePersisted, writePending],
  );
  ...
  const operationRecordColumns = useMemo<ColumnDef<SystemOperationRecord>[]>(
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <input
              ...
              className="console-operation-check"
              disabled={!canDeleteOperationRecords || !storagePersisted || writePending}
              type="checkbox"
      ...
      },
      {
        accessorKey: "path",
        cell: ({ row }) => (
          <div className="console-operation-request">
            <span className="console-api-method" data-method={row.original.method.toLowerCase()}>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className="console-operation-status"
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="console-operation-user">
      ...
      },
      {
        ...
        cell: ({ row }) => {
          return row.original.ipAddress ? (
            <code className="console-operation-code">{row.original.ipAddress}</code>
          ) : (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          return row.original.traceId ? (
            <code className="console-operation-code">{row.original.traceId}</code>
          ) : (
            t("common.labels.none")
          );
        },
  ...
  const openBulkDelete = () => {
    if (!canDeleteOperationRecords || !visibleSelectedIds.length) {
      return;
    }
  ...
  const confirmPendingDelete = () => {
    if (!pendingDeleteIds || !storagePersisted || !canDeleteOperationRecords) {
      return;
    }
    setNotice(null);
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-operation-records-title">
      <div className="console-admin-page-header">
        ...
        <div className="console-operation-page-actions">
          <Button
            ...
            title={
              canDeleteOperationRecords
                ? undefined
                : t("admin.operationRecords.states.deletePermissionDescription")
            }
          >
      ...
      {operationRecordsQuery.error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(operationRecordsQuery.error, t, operationRecordErrorCopy)}
          description={adminErrorDescription(
            operationRecordsQuery.error,
            t,
            operationRecordErrorCopy,
          )}
        />
      ) : null}
      ...
      {pendingDeleteIds ? (
        <StateBlock
          action={
            <div className="console-operation-confirm-actions">
              <Button
                disabled={!canDeleteOperationRecords}
      ...
      <div
        ...
        aria-label={t("admin.operationRecords.summaryLabel")}
      >
      ...
      <section className="console-admin-panel">
        ...
        {operationRecordsQuery.isLoading ? (
          ...
          <>
            ...
            <div className="console-operation-table">
              <DataTable
...
function OperationPayload({
  ...
  if (items.length === 0) {
    return (
      <span className="console-operation-muted">
        {t("admin.operationRecords.labels.noPayload")}
      </span>
    );
  }

  return (
    <div className="console-operation-payload">
      {items.map((item) => (
        <div key={item.label}>
...
function deleteErrorDescription(error: unknown, t: ReturnType<typeof useTranslation>["t"]) {
  if (error instanceof ApiError && error.status === 403) {
    return t("admin.operationRecords.states.deletePermissionDescription");
  }
  return adminErrorDescription(error, t, operationRecordErrorCopy);
}
