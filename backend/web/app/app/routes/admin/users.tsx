...
import { Tooltip } from "~/components/console/primitives/Tooltip";
...
const platformOwnerRoleCode = "platform_owner";
const userErrorCopy = {
  defaultTitle: "admin.users.states.errorTitle",
  permissionDescription: "admin.users.states.permissionDescription",
  permissionTitle: "admin.users.states.permissionTitle",
  storageUnavailableDescription: "admin.users.states.storageUnavailableDescription",
  storageUnavailableTitle: "admin.users.states.storageUnavailableTitle",
};
...
export default function AdminUsersRoute() {
  ...
  const canInviteUser = hasSessionPermission(permissions, {
    code: "user:invite",
    productCode: productCode || undefined,
    scope: "tenant",
  });
  const canUpdateUser = hasSessionPermission(permissions, {
    code: "user:update",
    productCode: productCode || undefined,
    scope: "tenant",
  });
  ...
  const updateMemberMutation = useMutation({
    ...
    onError: (error) => {
      setNotice({
        description: adminErrorDescription(error, t, userErrorCopy),
        intent: "danger",
  ...
  const userPermissionDescription = useCallback(
    (permission: string) =>
      t("admin.users.states.writePermissionDescription", {
        permission,
      }),
    [t],
  );
  const setUserPermissionNotice = useCallback(
    (permission: string) => {
      setNotice({
        description: userPermissionDescription(permission),
        intent: "danger",
        title: t("admin.users.states.permissionTitle"),
      });
    },
    [t, userPermissionDescription],
  );
  ...
  const assignableRoleItems = useMemo(
    () => roleItems.filter((role) => isAssignableUserRole(role)),
    [roleItems],
  );
  const assignableRoleOptions = useMemo<SelectOption[]>(
    ...
    [assignableRoleItems, t],
  );
  const selectedInviteRole = assignableRoleItems.some((role) => role.code === inviteDraft.roleCode)
    ? inviteDraft.roleCode
    : assignableRoleItems[0]?.code || "";
  ...
  const userColumns = useMemo<ColumnDef<IAMOrganizationUser>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="console-user-principal">
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const platformOwnerUser = hasPlatformOwnerRole(item.roles);
          const currentRole = platformOwnerUser ? "" : firstRoleCode(item.roles);
          const selectedRole = roleDrafts[userId] ?? currentRole;
            ...
            <div className="console-user-actions">
              <SelectField
                ...
                disabled={
                  ...
                  !canUpdateUser ||
                  platformOwnerUser ||
                  assignableRoleItems.length === 0
                }
                label={t("admin.users.fields.role")}
                options={
                  platformOwnerUser
                    ? [{ label: t("admin.users.roles.platformOwnerLocked"), value: "" }]
                    : assignableRoleOptions
                }
                value={platformOwnerUser ? "" : selectedRole}
                onChange={(event) => {
              ...
              <Button
                ...
                icon={<Save size={16} />}
                title={!canUpdateUser ? userPermissionDescription("user:update") : undefined}
                onClick={() => {
                  if (!canUpdateUser) {
                    setUserPermissionNotice("user:update");
                    return;
                  }
                  updateMemberMutation.mutate({ item, roles: [selectedRole] });
                }}
              >
              ...
              <Button
                ...
                disabled={updateMemberMutation.isPending || !canUpdateUser || platformOwnerUser}
                ...
                onClick={() => {
                  ...
                  updateMemberMutation.mutate({ item, status: nextStatus });
                }}
              >
                {nextStatus === "active"
  ...
  const invitationColumns = useMemo<ColumnDef<IAMInvitation>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => <code className="console-user-code">{row.original.roleCode}</code>,
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="console-user-invitation-actions">
              <Button
                appearance="secondary"
                ...
                disabled={
                  status !== "pending" || revokeInvitationMutation.isPending || !canInviteUser
                }
                icon={<Ban size={16} />}
                title={!canInviteUser ? userPermissionDescription("user:invite") : undefined}
                onClick={() => {
                  if (!canInviteUser) {
                    setUserPermissionNotice("user:invite");
                    return;
                  }
  ...
  const inviteButton = (
    <Button
      disabled={!currentOrgId || !canInviteUser}
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-users-title">
      <div className="console-admin-page-header">
        ...
        <div className="console-user-page-actions">
          {!currentOrgId ? (
            <Tooltip content={t("admin.users.states.missingOrgDescription")}>
              <span className="console-tooltip-anchor">{inviteButton}</span>
            </Tooltip>
          ) : (
            inviteButton
          )}
      ...
      {firstError ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(firstError, t, userErrorCopy)}
          description={adminErrorDescription(firstError, t, userErrorCopy)}
        />
      ) : null}
      ...
      {delivery?.debug ? (
        <section className="console-admin-panel console-user-delivery-panel">
          <header>
            <h2>{t("admin.users.invite.debugDeliveryTitle")}</h2>
            <p>{t("admin.users.invite.debugDeliveryDescription")}</p>
          </header>
          <div className="console-user-delivery-grid">
            {delivery.token ? (
              <div>
      ...
      <Dialog
        ...
        footer={
          <div className="console-user-confirm-actions">
            <Button
      ...
      >
        <form className="console-user-invite-form" onSubmit={submitInvite}>
          <FormField
            disabled={!canInviteUser || inviteUserMutation.isPending}
          ...
          <SelectField
            disabled={
              !canInviteUser || inviteUserMutation.isPending || assignableRoleItems.length === 0
            }
          ...
          />
          <div className="console-user-invite-actions">
            <Button
              disabled={!canInviteUser || !selectedInviteRole}
              icon={<Send size={17} />}
      ...
      <div className="console-admin-stat-grid" aria-label={t("admin.users.summaryLabel")}>
        <UserStatCard
          icon={<Users size={19} />}
      ...
      >
        <form
          className="console-admin-filter-form console-user-filter-form"
      ...
      <section className="console-admin-panel">
        <header className="console-admin-panel-header-row">
          ...
          <div className="console-admin-pager" aria-label={t("admin.users.pagination.label")}>
            <Button
              appearance="secondary"
        ...
        {usersQuery.isLoading ? (
          ...
          <>
            ...
            <div className="console-user-table">
              <DataTable
                columns={userColumns}
                data={pageData.items}
      ...
      <section className="console-admin-panel">
        ...
        {invitationsQuery.isLoading ? (
          ...
          <div className="console-user-invitation-table">
            <DataTable
...
function cleanRoleCode(value: string) {
  return value.trim().replace(/^role:/i, "");
}

function hasPlatformOwnerRole(roles: string[]) {
  return roles.some((role) => cleanRoleCode(role) === platformOwnerRoleCode);
}

function isAssignableUserRole(role: IAMRole) {
  return cleanRoleCode(role.code) !== platformOwnerRoleCode;
}
...
function RoleList({ roles }: { roles: string[] }) {
  ...
  return (
    <div className="console-user-role-list">
      {roles.map((role) => (
