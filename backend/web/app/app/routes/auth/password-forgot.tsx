...
import { createForgotPasswordSchema, type ForgotPasswordFormValues } from "~/features/auth/schemas";
...
export default function PasswordForgotRoute() {
  ...
  return (
    <main className="console-auth-page">
      <section className="console-auth-card" aria-labelledby="password-forgot-title">
        ...
        <ConsoleForm form={form} onSubmit={onSubmit}>
          <ConsoleTextField<ForgotPasswordFormValues>
            autoComplete="email"
...
function PasswordDebugDelivery({ delivery }: { delivery: NotificationDelivery }) {
  ...
  return (
    <div className="console-auth-debug" aria-label={t("auth.passwordForgot.debugTitle")}>
      {delivery.token ? (
        <div>
