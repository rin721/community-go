...
import { FormSkeleton } from "~/components/console/patterns/LoadingSkeletons";
import { StateBlock } from "~/components/console/patterns/StateBlock";
import {
  StepWizard,
  type StepWizardItem,
  type StepWizardStatus,
} from "~/components/console/patterns/StepWizard";
import {
...
export default function SetupWizardRoute() {
  ...
  return (
    <main className="console-setup-layout">
      <aside className="console-setup-steps">
        <StepWizard
      ...
      <section className="console-setup-panel" aria-labelledby="setup-step-title">
        <header className="console-setup-panel__header">
          <Badge>
            {activeReport
        ...
        <div className="console-setup-actions">
          <Button
...
function SetupRunLogs({ steps }: { steps: InitializationStep[] }) {
  ...
  return (
    <section className="console-setup-log-panel" aria-labelledby="setup-logs-title">
      <h2 id="setup-logs-title">{t("setup.logs.title")}</h2>
      <ol className="console-setup-log-list">
        {steps.map((step) => (
...
function SetupReportSummary({ report }: { report: InitializationReport }) {
  ...
  return (
    <section className="console-setup-report" aria-labelledby="setup-report-title">
      <h2 id="setup-report-title">{t("setup.report.title")}</h2>
      <p>{report.summary}</p>
      <dl className="console-setup-report__stats">
        <div>
...
function SetupStepReport({ step }: { step: InitializationStep }) {
  ...
  return (
    <section className="console-setup-report" aria-labelledby="setup-step-report-title">
...
function SetupTestFeedback({ current, result }: { current?: boolean; result: SetupTestResult }) {
  ...
  return (
    <section
      className="console-setup-test-feedback"
    ...
    >
      ...
      {result.repairHint ? (
        <p className="console-setup-test-feedback__hint">
          {t("setup.test.repairHint")}: {result.repairHint}
        </p>
      ) : null}
      {current === false ? (
        <p className="console-setup-test-feedback__hint">{t("setup.test.stale")}</p>
      ) : null}
      {result.restartRequired ? (
        <p className="console-setup-test-feedback__hint">{t("setup.test.restartRequired")}</p>
      ) : null}
    </section>
  );
}
...
function SetupLanguageStep({
  ...
  return (
    <div className="console-form-field">
      ...
      <span className="console-form-field__help">{t("setup.language.description")}</span>
    </div>
  );
}
...
function SetupStepForm({
  ...
  return (
    <div className="console-setup-form">
      {groups.map((group) =>
        isVisible(group.visibleWhen, values) ? (
          <fieldset className="console-setup-fieldset" key={group.key}>
...
function SetupField({
  ...
  if (field.type === "boolean") {
    return (
      <label className="console-setup-check">
        <input
          id={fieldId}
...
function SetupRunConfirmation({
  ...
  return (
    <div className="console-setup-check console-setup-confirm">
      <input
        id={inputId}
        checked={checked}
        type="checkbox"
      ...
      <span>
        ...
        <span className="console-form-field__help">{t("setup.confirm.description")}</span>
      </span>
    </div>
  );
}
