...
func TestProcessGeneratedConfigurationSupportsMigrationAndServiceStartup(t *testing.T) {
	...
	if decodeErr != nil || len(beforeMigration.Sets) != 5 || !allEmpty || beforeMigration.Compatible {
	...
	if decodeErr != nil || len(afterMigration.Sets) != 5 || !allCompatible || !afterMigration.Compatible {
