...
| WebUI 组件库 | 059/062/067 建立自研 UI 原语（`webui/src/ui`）与 `styles.css` public UI 分区；068 按用户指令全量采用 HeroUI v3（`@heroui/react 3.2.4 / @heroui/theme 2.4.26 / @heroui/toast 2.0.22 / @heroui/styles 3.2.4`，均 MIT；peer React≥19、tailwindcss≥4） | **采用 HeroUI v3 作为组件库（068 已实施），自研原语退役为平台契约层** | HeroUI 只在 `webui/src/sdk/ui`（`@webui/sdk/ui`）内部使用，模块不直接依赖；平台契约（ActionTrigger 权限呈现、zone 注入、Reveal、滚动运行时、experience 派生配置、reduced-motion）保留自研；Button/Input/Select/Chip/Alert/Card/Table/Pagination/Toast/Skeleton/EmptyState/IconButton 等由 HeroUI 承担；遮罩容器（ConfirmDialog/Drawer/ThemeDrawer/RouteSearch）与 Switch/Checkbox 控件因 v3 复合组件不含交互 input 或 SSR 为空输出，暂保留自绘并记录边界（R068-002）；样式 authority 收窄为 reset/token/平台行为 + Tailwind v4 + `@heroui/styles` 静态组件样式；暗色联动为 `<html>`.dark class |
