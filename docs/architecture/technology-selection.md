...
| HTTP 契约/OpenAPI | `pkg/httpx/contract` 约 1,069 行自研 typed DSL/renderer/codec；运行时又把文档交给 kin-openapi 校验 | **采用 Huma v2，第一片已通过、全量迁移中** | `Huma v2.39.1` 已接管 `login`、`listTodos`、`organization.departments.update` 的 typed input/output、OpenAPI/JSON Schema、validation 与 chi registration；项目保留 OperationGate、Problem、operation/policy metadata、生成 inventory 和 server lifecycle。`HTTP-057-002` 完成后删除自研 contract/codec/dispatcher 与重复 kin-openapi validation；ogen 仅在未来改为 spec-first 时重评 |
