...
| Permission/AuthZ | code-defined Permission Catalog + IAM typed 表存储 Core RBAC；`Casbin v3.10.0`（Apache-2.0，当前稳定版）只在 `internal/module/iam/adapter/casbin` 内执行固定两字段 Core RBAC 求值；Auth 只依赖消费方 `DecisionPoint` port，Principal 显式区分 `token-scopes`/`iam-rbac`；authorization revision 与不可变 snapshot 保证跨 Generation/并发 mutation fail closed | **采用 Casbin v3 作为 IAM 内 evaluator（058 已单轨实施）；IAM 继续是唯一业务与存储 authority** | Casbin model/policy 固定为 exact Core RBAC，不开 hierarchy/domain/deny/wildcard/ABAC/ReBAC/superuser；不使用 GORM Adapter、AutoLoad、Watcher 或 AI API。出现真实 domain/ABAC 语义时重新研究 model 与存储再决策；跨资源 ReBAC/集中决策继续比较 `OpenFGA`/OPA，不因 058 改变立场 |
...
| WebUI 组件库 | 059/062/067 建立自研 UI 原语（`webui/src/ui`）与 `styles.css` public UI 分区；068 按用户指令全量采用 HeroUI v3（`@heroui/react 3.2.4 / @heroui/styles 3.2.4`，均 MIT；peer React≥19、tailwindcss≥4）；090 已单轨移除 HeroUI v2 的 `@heroui/theme` 与 `@heroui/toast`（Toast 改走 v3 队列 API）；069 把 Shell/遮罩/表单控件由 HeroUI/RAC 拼装（显式依赖 `react-aria-components 1.20`，MIT，为 HeroUI v3 自身底座） | **采用 HeroUI v3 作为组件库（068 已实施）；090 单轨 v3 无 v2 残留；069 起 Shell 与遮罩/开关/复选框以 HeroUI/RAC 拼装** | HeroUI 只在 `webui/src/sdk/ui`（`@webui/sdk/ui`）内部使用，模块不直接依赖；平台契约（ActionTrigger 权限呈现、zone 注入、Reveal、滚动运行时、experience 派生配置、reduced-motion）保留自研；Button/Input/Select/Chip/Alert/Card/Table/Pagination/Toast/Skeleton/EmptyState/IconButton 由 HeroUI 承担；遮罩（ConfirmDialog/Drawer/ThemeDrawer/RouteSearch）与 Switch/Checkbox 由 RAC 受控 Modal/Dialog/Switch/Checkbox 承担（v3 复合组件无交互 input、Modal 为 DialogTrigger 客户端模式，故显式依赖 RAC 底座、视觉对齐 HeroUI）；样式 authority 收窄为 reset/token/平台行为 + Tailwind v4 + `@heroui/styles` 静态组件样式；暗色联动 `<html>`.dark；RAC overlay 单测走客户端渲染（`renderClient`），e2e 以 role/aria 为最终门禁（R068-002） |
