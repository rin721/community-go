...
| UI-002-COMMON-001 | UI-002, I18N-CONTRACT-001 | 落地公共管理模式的可复用宿主组件 | `DataToolbar`、`FilterPanel`、`DataTable`、`Pagination`、`EmptyState`、`InlineAlert`、`Drawer` 由模块注入翻译/业务内容；toolbar label、选择 mixed 状态、加载、空态、分页、筛选 region 语义和 Drawer 焦点边界有测试；不注册 Demo 路由 | 已实施未闭合 |
