...
- cache 深化追踪确认当前没有 production typed L1 消费者，默认 L1 又缺少容量上界和跨实例失效；修订计划将先退役 L1 与 `patrickmn/go-cache`，不为未量化需求立即引入另一缓存库。Otter v2/ttlcache v3 保留为满足明确收益、内存和陈旧预算后的候选。
