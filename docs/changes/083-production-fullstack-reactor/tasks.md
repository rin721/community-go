...
| `PAGE-083-007` | M | 确认 | P4：危险确认（Accounts/Roles/ApiTokens/Departments archive/revoke → Confirm/DangerZone）+ 空载态规格 | 确认 e2e；Empty/Loading 组件采用 | 部分完成（确认、主要列表空态/loading、五个主要列表 connectivity 错误态及统一 Retry 动作已完成；详情错误态与确认 e2e 仍待补） |
...
| `QA-083-001` | M | 上述 | 页面达标率复核（R083-003 105 项重跑，目标 ≥70% 达标或收敛）+ 三层 QA | 达标率提升记录 | 部分完成（已确认基线口径为 105 项、30 达标/60 部分/15 重做；mock E2E 3/3、全量 Vitest 199、lint/build 已通过；当前五个列表错误态均可 Retry；真实后端/移动视口与逐项重跑记录待补） |
