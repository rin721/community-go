...
- **084d 残余（第五批继续）**：关于/致谢等静态页卡片高度与留白观感（内容驱动的有界静态页，评审偏严格）、仪表盘首屏折线与完整指标区（长页面滚动为正常形态）、FilterBar 多控件行的分组/间距观感、「…」更多操作按钮边界与可发现性、列表卡标题区垂直留白（移动视口验证）。均不阻断使用，按 R084-001 复跑收敛。
- 移动视口与真机验证沿用既有受限项。