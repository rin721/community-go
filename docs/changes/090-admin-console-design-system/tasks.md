...
- [ ] PATTERN-090-003 实现 BatchOperation 与统一 Feedback matrix；前置：PATTERN-090-001；当前增量：账户批量操作已复用 `BatchResultSummary`，服务端同步结果现包含请求/处理/成功/失败集合、稳定错误码、retryable 和 correlation ID，`Idempotency-Key` 可安全重放；运维诊断失败已复用 `ErrorState` 并投影 request id。API Token 批量吊销已接入统一 `BatchResultSummary` 反馈（汇总 + 逐项失败，替代页面自拼 InlineAlert），与账户批处理 feedback matrix 一致；完整 preview/running/partial 状态机及 P1/P2 异步 Job 适配仍待补齐。
...
- [ ] PATTERN-090-005 以真实指标完成 Statistic/Chart spike 并实现可解释可视化契约；前置：BE-090-001；当前证据：Dashboard 运维卡片已将标量响应投影为结构化事实列表，避免主界面直接倾倒原始 JSON；Monitoring 只展示已有进程指标，未接入的宿主磁盘/网络遥测改为说明文本，不再伪装为同等重要数据卡片。typed 指标契约已落地（090 design §8）：`/management/metrics-summary` 返回稳定 key/value/unit/asOf，WebUI 不再解析 Prometheus 文本推断语义；真实历史指标、数据口径文档、可访问数据表与 bundle 测量仍待补齐。
...
- [ ] PAGE-090-002 迁移角色、权限、会话和 API Token；前置：PAGE-090-001；当前证据：角色管理区已使用 `EntityDetail`、统一 `Pagination` 与 URL 分页状态，现由 `iam.roles.read` 聚合详情展示受影响账户、权限/模块范围和关键/敏感风险计数，权限矩阵逐项显示 Catalog 风险元数据并具备加载/Problem 重试态；权限目录具备统一查询反馈，会话与 API Token 列表已接入服务端 `offset/limit/total` 分页及页码/筛选 URL 状态。Token 批处理已落地：`POST /api/v1/iam/api-tokens/batch-revoke`（`BatchRevokeApiTokensIdempotent`，Idempotency-Key 稳定重放、已吊销幂等、逐项成功/失败集合），ApiTokensPage 列表支持多选 + `BulkActionBar` 批量吊销 + 逐项失败反馈；一次性 secret 完整流程已有 mock E2E 覆盖（创建 → 展示 secret → Rotate → 新 secret + 批量选择出现操作条）；service/HTTP 集成测试覆盖批量吊销幂等与冲突语义；响应式视觉矩阵仍待补齐。
- [ ] PAGE-090-003 迁移审计；前置：PATTERN-090-001、BE-090-003；当前证据：筛选器已覆盖 operation/action/outcome/actor/subject/resource/correlation/time range，事件列表与详情均展示可复制的低敏 correlation ID；详情抽屉由独立服务端投影驱动，保留低敏字段与 JSON 视图，并覆盖 loading/Problem/retry；分页已从 offset 迁移到服务端游标（`cursor`/`nextCursor`/`hasMore`），页面提供会话内上一页/下一页导航与每页条数切换，筛选变化回到第一页。完整查询测试已补齐：sink 集成测试新增「游标 + 筛选组合」（operation/outcome 过滤下 keyset 翻页不重不漏、只返回满足筛选事件、全量总数对比）与既有 7 条同刻事件游标翻页/无效游标 fail closed；视觉矩阵仍待补齐。
- [ ] PAGE-090-004 迁移组织 Tree、岗位和任职；前置：PATTERN-090-002；当前证据：部门 Tree、岗位目录和账号任职页均使用统一 PageFrame/分栏骨架，任职保存条已呈现 clean/dirty/pending/conflict 状态，组织管理 E2E 覆盖树、岗位与任职编辑。移动预检已落地：390px 下 split-workspace 折叠单栏，部门树选择/详情、岗位表格、任职账号选择均可交互且三页无横向溢出（新增 mock E2E「090 organization pages remain usable on compact viewport」）；成员视图（部门→账号反向成员查询）属 P1 后端契约（R090-003「反向成员查询」），仍待独立研究。
...
- [ ] PAGE-090-006 重构 Dashboard 与系统状态；前置：SHELL-090-002；当前证据：运维能力卡片已从原始 JSON 改为有限标量事实摘要，并保留加载/失败/重试状态；监控区不再将未接入的宿主磁盘/网络指标伪装成同等重要卡片，而是明确保留接入说明。真实数据来源与口径已落地：新增 `/management/metrics-summary` typed 指标投影（`pkgobservability.Metrics.Summary` 从进程 registry gather 提取稳定 key/value/unit/asOf，按 key 稳定排序，采样失败返回错误不返回部分数据），WebUI Dashboard/Monitoring 改消费 typed 投影，删除 Prometheus 文本解析（`metrics-data.ts` 重写为 typed 读取）；ops handler 测试、kernel observability Summary 测试、WebUI metrics-data/mock 测试覆盖；视觉矩阵与人工审阅记录仍待补齐。
...
- [ ] VERIFY-090-001 执行单元、组件、集成、E2E、类型、lint、构建和 Go 侧适用验证；前置：全部 P0 实施任务；当前证据（最近一轮全量）：Go `build`/`vet`/`test ./...` 全绿、`webui generate --check` 通过；WebUI `typecheck`、`vitest`（251 tests）、`eslint`（0 errors，仅 4 条既有测试文件 warning）、`lint-architecture`、`build` 全绿；mock Playwright E2E 19/19 通过（含 090 系列：视口矩阵 12 页面族 × 5 档、暗色/compact 截图、审计游标/空态、偏好持久化、OpenAPI 三段式、API Token 一次性 secret、组织移动预检、键盘/读屏语义）；`git diff --check` 干净。
- [ ] VERIFY-090-002 完成 1440/1280/1024/768/390、亮暗、normal/compact 和完整状态的多模态视觉复核；前置：全部页面迁移；当前证据：`webui/e2e/webui-mock.spec.ts` 的 090 矩阵测试已覆盖全部 12 个代表页面族（Dashboard、账户、角色、部门、岗位、会话、API Token、设置 Profile/Appearance/Notifications、审计、OpenAPI）× 5 档视口（1440/1280/1024/768/390），逐页断言无横向页面溢出且 shell 宽度非零；截图证据覆盖 Dashboard 亮色、Dashboard/设置 Appearance/账户列表暗色+compact（新增独立测试「090 dark compact snapshots for settings and accounts」）。完整状态已落地：审计页操作筛选无匹配时渲染表格空行空态、清空筛选恢复数据（新增 mock E2E「090 audit page shows empty state when filters match nothing」，mock 路由现按 operation/outcome 过滤并与服务端语义一致）；loading/error 状态沿用既有组件单测与页面重试态。人工审阅记录仍待补齐。
- [ ] VERIFY-090-003 完成键盘、焦点、读屏语义、对比度、长文本和高密度数据验证；前置：全部页面迁移；当前证据：自动化键盘/读屏基线已覆盖——账户列表 DataTable `aria-label`/`aria-busy` 与筛选可聚焦（Enter 提交后列表仍可达）、部门树 `role=tree` 与树节点可聚焦、OpenAPI 标签栏 `role=tablist`（新增 mock E2E「090 migrated pages expose keyboard and screen-reader semantics」）；宿主侧既有 WorkspaceTabs roving focus（Left/Right/Home/End）、Tree role、StatusBadge progressbar/复制 aria-label、MockBadge role=status 等单测；对比度、长文本与高密度数据的人工审阅记录仍待补齐。
- [ ] CLEANUP-090-001 搜索并删除旧 UI、旧 Token、HeroUI v2、旧 CSS、旧工作区表现和失效文档；前置：VERIFY-090-001；当前证据：HeroUI v2/`@nextui`/v2 theme plugin 零引用（package.json 仅 v3），`org-tree-inspector` 旧双栏布局与 `.app-footer`/`.app-footer-zones` 死 CSS（083 移除固定 Footer 后无消费者）已删除；`workspace-panel-scroll` 双 padding、`data-page-width` 死规则已在 086 移除；WebUI 侧无调用方的 `loadMetrics`（Prometheus 文本客户端）与 `requestText` 导入已删除（raw text 端点保留给监控系统）；`src/contracts/data-source.test.ts` 显式 stub server-hosted 消除跨测试文件环境泄漏（pre-existing flaky 修复）；完整 diff 与剩余旧引用仍待最终扫描。
- [ ] DOC-090-001 同步 WebUI 权威文档、OpenAPI、变更导航和视觉基线；前置：CLEANUP-090-001；当前证据：`docs/development/webui.md` 移除 HeroUI v2 依赖描述（`@heroui/theme`/`@heroui/toast`）并新增「成熟后台控制台（090）」权威小节（页面骨架/模式组件、一致查询、跨设备偏好、typed 指标、OpenAPI 紧凑化、单轨清理）；management facade 路径清单与 `docs/getting-started/webui.md`、`docs/operations/runtime-capabilities.md` 同步 `/metrics-summary`；`docs/architecture/technology-selection.md` HeroUI 行记录 090 单轨 v3；OpenAPI/registry 重新生成；docs-guard 通过；视觉基线（VERIFY-090-002 截图）与剩余页面族人工审阅仍待补齐。
