...
- [075 新增 openapi 模块：WebUI 可视化渲染 API 契约](075-openapi-webui-render/README.md)：已确认并实施完成（55ee70f → 9ea2f13 → 第三轮工作台）；按用户三轮要求演进：swagger-ui-react 渲染（55ee70f）→ 平台组件只读参考页并移除 swagger（9ea2f13）→ **Apifox 风格可测试工作台（R075-004）**：操作树导航 + 详情 + 同源 fetch 执行（bearerAuth 内存 token、webuiSession Cookie+CSRF）+ 响应面板 + 模型视图，`?view=&op=` 深链，mock 执行禁用；契约快照生成链不变；顺带修复平台 DataTable 客户端渲染缺陷（RAC isRowHeader）。
