...
- [082 WebUI 产品架构与 UI 体系重构](082-webui-architecture-rebuild/README.md)：已确认，实施中；以 temp-new-changes.md 方案（81 章「前端产品能力系统性重构」，commit 3b758bd，旧 80 节编号映射见 R003 §4.11）为输入，R001/R002/R003 完成现状审计（24 路由/25 菜单/55 operation/23 权限键）与差异分析（已满足 28 节/部分满足 42/未满足 4/候选 6）；计划按「平台底座（DataTable 增强/FilterBar+URL 同步/FormField/状态反馈体系/语义组件/token 补齐/Query-Mutation 统一层/Backend 错误分类/Adapter 层）→ 页面迁移（IAM/Audit/Ops/Org/Navigation/Session/Tokens）→ 打磨验收（三层 QA）」与新版方案对齐，明确否决后端不存在能力（fake 红线），决策点 DEC-082-001..006 已确认，实施从平台底座开始分批提交。

下一个任务序号为 `083`。已完成记录只保存历史证据；当前行为必须回到根 [README](../../README.md) 和对应主题文档确认。
