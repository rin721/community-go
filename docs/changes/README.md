...
- [073 全局菜单两级化 + 分组布局（固定页内导航）](073-two-level-settings-menu/README.md)：已确认并实施（进行中，验证已绿）；全局菜单收敛为两级（顶级「设置」→ 五主分区子项，移除 host.center 装配与 locale 键）；契约新增 `Route.GroupLayoutID` 分组布局——宿主 `ModuleGroupLayout`（懒加载布局 + Outlet children 注入）承载 settings 八路由共享 `SettingsLayout`（固定 SectionNav + 内容区，切换不重挂，e2e dataset 标记断言）；HostNavigation 契约能力保留。

下一个任务序号为 `074`。已完成记录只保存历史证据；当前行为必须回到根 [README](../../README.md) 和对应主题文档确认。
