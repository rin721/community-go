...
- [075 新增 openapi 模块：WebUI 可视化渲染 API 契约](075-openapi-webui-render/README.md)：研究门禁已通过（R075-001/R075-002/R075-003）；首版（swagger-ui-react 渲染）已提交 `55ee70f`，用户追加要求「页内组件也使用当前 WebUI 组件」后计划已按 R075-003 更新（平台组件自绘只读参考页、移除 swagger-ui-react），重新待确认；契约快照生成链不变。
