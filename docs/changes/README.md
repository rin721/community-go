...
- [075 新增 openapi 模块：WebUI 可视化渲染 API 契约](075-openapi-webui-render/README.md)：已确认并实施完成（55ee70f → 9ea2f13）；页内呈现按用户要求改为 `@webui/sdk/ui` 平台组件自绘只读参考页（operation tag 分组/参数·响应表/schema 属性表），移除 swagger-ui-react 及其依赖/别名（R075-003 取代 R075-001 页内选型）；契约快照生成链（`webui generate` → openapi-spec.ts + `--check`）不变；顺带修复平台 DataTable 客户端渲染缺陷（RAC isRowHeader）；OpenAPIPage chunk 由 1.37MB 降至 46KB（gzip）。
