...
- [048 业务模块自有 WebUI 与通用 SDK 重构](048-full-stack-webui-modules/README.md)：已按最新决策修订；页面继续由 `internal/module/<id>` 持有，普通模块 core 零修改，只有真实新宿主能力才单独增加 SDK interface/adapter；Selection/Activation/Delivery/Availability/Access 逐层 fail closed，未启用、未交付或不可用内容不得触发业务资源加载；非文档实施待确认。
