...
- [053 账号、组织与权限体系业务模块](053-account-access-management/README.md)：研究门禁已通过，计划按首发前语境修订并重新待确认；拟新增 Account 业务模块，单轨接管本地账号/凭据/Session，保留解耦的 Todo 学习示例，并建立用户、角色权限、受控菜单策略、部门岗位、管理 API/WebUI 与 Todo/Account 干净 migration baseline。
