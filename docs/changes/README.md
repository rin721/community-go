...
- [075 新增 openapi 模块：WebUI 可视化渲染 API 契约](075-openapi-webui-render/README.md)：按用户四轮要求演进至**Apifox 复刻 API 管理平台（R075-005，HeroUI 控件基座）**：55ee70f swagger 渲染 → 9ea2f13 平台组件参考页 → e4865ca 可测试工作台 → 本轮 Apifox 形态（资源树/多标签/文档调试双模式/在线调试/响应面板/模型/Cmd+K/深链 `?op=&mode=`），JSON 高亮 highlight.js；像素级还原以截图逐轮人工校准；契约快照链不变。实施验证与提交收尾中。
