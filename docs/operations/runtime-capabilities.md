...
| 机器访问令牌（078） | `iam:api-token:read/write`、`/api/v1/iam/api-tokens*`、`iam_api_tokens` 表（migration 000006）、Auth `ChainVerifier`（JWT→API-Token） | IAM：secret 只存 sha256、明文仅创建/轮换一次；Auth Bearer 链解析 token-scopes Principal，授权路径零改动 | Go 单元（发证/列表/轮换/吊销/解析/未知 scope 拒绝/审计无明文）、migration 三驱动、WebUI Vitest | 跨工具（CLI）常态体验、按令牌配额/用途分组（候选） |
