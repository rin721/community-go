...
| Database/Migration | `db migrate status/up`、database/migration 文档 | Migration one-shot 与数据库组件 | SQLite 本地路径、Go 测试 | 生产数据库协议需独立证据 |
...
| Admin WebUI | `webui/`、WebUI 本地启动指南 | Vite 本地开发服务器；后端负责 API/Auth | generate、lint、typecheck、test、build | Docker/release 静态资源托管、E2E、视觉验收 |
