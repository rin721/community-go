...
test('无 Suspense 页面深入导航播放完整方向滑动并清理临时样式', async ({ page }) => {
  ...
  expect(record.animationNames).toContain('page-slide-in');
  expect(record.animationNames).toContain('page-slide-out');
...
test('浏览器后退不播放方向滑动并正确恢复页面', async ({ page }) => {
  ...
  expect(record.animationNames).not.toContain('page-slide-in');
  expect(record.animationNames).not.toContain('page-slide-out');
});
