...
- `layout`：AdminPage、Header、Toolbar、Filter、Section、Split View、Sticky Actions。`AdminSection` 是 Panel 类 Section 容器（header + children）；需要「章节导航 + 内容共享父容器内容边界」的组合（如 `TabsView variant="section"` 作章节导航）时使用 `contentInset`（children 包进统一 horizontal inset）或 `AdminSectionBody`（自定义 header 的 Panel body 复用同一 inset）。父容器负责区域分隔（divider/boundary）；Tabs 等子组件不自建 Surface。
