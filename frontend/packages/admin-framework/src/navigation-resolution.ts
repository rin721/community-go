/**
 * Admin Framework —— Navigation Resolution（Sidebar 模型单一 authority）。
 *
...
export function resolveNavigationWithPlugins(
  ...
  const routeHref = (
    routeId: string,
  ): { href: string | undefined; diagnostics: AdminDiagnostic[] } => {
...
function aliasSortKey(a: AdminNavigationGroupAlias, b: AdminNavigationGroupAlias): number {
