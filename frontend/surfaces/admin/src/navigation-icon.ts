/**
 * Admin Surface —— Navigation Icon Vocabulary Authority。
 *
 ...
 * 功能能力，也不是能力协商。合法 icon 集合由 Admin Surface 治理（本模块），
 * Registry/Framework 只透传 opaque id，Shell Sidebar 按自己的 presentation
 * policy 决定是否及如何消费。
 *
 * 本模块只承担：
 * - semantic icon vocabulary（adminNavigationIconVocabulary）；
 * - vocabulary types；
 * - icon reference 的纯 validation / diagnostic。
 *
 * 无副作用、纯 TS；`shell-model`/`tooling/admin-codegen`/Host Shell resolver
 * 均消费本 authority。
 */
 ...
 *
 * 来源：现有 `apps/admin-web/src/shell/navigation-tree.tsx` 的 iconByNavigationId
 ...
 */
export const adminNavigationIconVocabulary = [
  'dashboard',
  'foundations',
  'catalog',
  'action',
...
/** 合法 semantic icon id。 */
export type AdminNavigationIconId = (typeof adminNavigationIconVocabulary)[number];
...
export type AdminNavigationIconReference = Readonly<{
  iconId: string;
  routeId: string;
}>;

/** 未知 icon 诊断：同一未知 iconId 聚合其声明 Route。 */
export type AdminNavigationIconDiagnostic = Readonly<{
  code: typeof UNKNOWN_ADMIN_NAVIGATION_ICON;
  iconId: string;
...
export function collectUnknownNavigationIconDiagnostics(
  references: readonly AdminNavigationIconReference[],
  vocabulary: readonly string[] = adminNavigationIconVocabulary,
): readonly AdminNavigationIconDiagnostic[] {
  ...
  return [...byIcon.entries()]
    ...
    .map(([iconId, routeIds]) => ({
      code: UNKNOWN_ADMIN_NAVIGATION_ICON,
      iconId,
      routeIds,
      message: `navigation.iconId 未命中 Admin Surface icon vocabulary: ${iconId}（声明于 ${routeIds.join(', ')}）`,
    }));
}
