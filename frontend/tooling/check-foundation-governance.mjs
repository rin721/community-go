...
// Generated Artifact 首行门禁：tokens.css 由 tooling/token-codegen 生成，
// 人工修改（首个非空行不等于 generated header）必须被 Gate 检测。
const generatedArtifacts = [
  {
    path: join(frontendRoot, 'packages', 'design-system', 'src', 'tokens.css'),
    header:
...
for (const artifact of generatedArtifacts) {
  if (!(await exists(artifact.path))) {
    violations.push(
      `${relative(frontendRoot, artifact.path)}: Generated Artifact 缺失（运行 pnpm codegen:tokens）`,
    );
    continue;
  }
  ...
  if (firstMeaningfulLine !== artifact.header) {
    violations.push(
      `${relative(frontendRoot, artifact.path)}: Generated Artifact 被人工修改（首行必须为 generated header；运行 pnpm codegen:tokens 重新生成）`,
    );
  }
}
