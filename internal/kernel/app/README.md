...
- `app/observability`：Metrics 使用 process-stable `ManagedFixed` Lease facade；Telemetry 使用 generation-owned `ManagedConfigured`，HTTP 请求在 Lease 内由官方 `otelhttp` 完成单一 server span，排空后 flush/shutdown；稳定 operation、低敏 route、项目 Prometheus 与 trace ID bridge 仍由项目治理，两者都不导出 Prometheus Registry、OTel Tracer/Provider 或关闭权。
- `app/execution`：配置化（memory/disabled）的后台任务执行组件，输出 `pkg/execution.OperationExecutor` 稳定 facade。命名 policy 明确 attempts、jitter、单次与总 timeout；`backoff/v7` 只在 pkg 内部承担 retry loop。memory backend 同步记录且只在单进程内可见；`Access.Health()` 只表达 enabled/ready。注入 Logger 只记录受控的 Debug retry 事件，不存在恢复探测、异步记录 goroutine 或虚构的外部 primary。
