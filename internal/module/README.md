...
当前已有 [IAM](iam/README.md)、[Organization](organization/README.md)、[Navigation](navigation/README.md)、[Auth](auth/README.md)、[Ops](ops/README.md)、[Migration](migration/README.md) 与 [Todo](todo/README.md) 模块。IAM 拥有本地身份、凭据、Session 与 Core RBAC，Organization 拥有部门、岗位与账号组织关系，Navigation 拥有已注册菜单的运行策略，Auth 拥有通用认证/授权/审计执行，Ops 拥有 management、探针和诊断用例，Migration 编排显式多 set status/up，Todo 拥有业务实体、对象授权 port 与 SQL migration set；composition 只连接完成品：
...
HTTP 模块遵循固定的代码优先源头与分层：模块顶层 `handler/` 承载 HTTP 应用语义适配（`Operations`/`Handler`、DTO 与映射、错误呈现、`ActorAccess`），`binding/http` 拥有 Huma typed input/output 与无资源 registration；`internal/composition` 显式聚合 registration 与运行期 handler；`internal/transport/http` 统一安装 OpenAPI 校验、operation gate、项目 Problem 和 chi route。Huma 核心包仅允许进入 `binding/http`，Huma Router adapter 由 `internal/transport/http/humabinding` 隔离，`handler`、Service 与 Model 不感知框架。057 第一片只迁移 `login`、`listTodos`、`organization.departments.update`，旧 `pkg/httpx/contract`/dispatcher 暂时承载其余 operation 并将在 `HTTP-057-002` 删除；新增模块不得复制 Router、route binding、method/path 表或继续扩展旧 DSL。
