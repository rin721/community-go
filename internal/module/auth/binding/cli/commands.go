...
// Commands 返回离线 WebUI 用户维护命令。
func (c *Contract) Commands() ([]cli.CommandSpec, error) {
	return []cli.CommandSpec{{
		Name: "webui", Description: "管理本地 WebUI 用户", Mode: cli.CommandModeApplication,
...
func (c *Contract) resetPasswordCommand() cli.CommandSpec {
	return cli.CommandSpec{
		Name: "reset-password", Description: "重置本地 WebUI 用户密码", Mode: cli.CommandModeApplication,
		SideEffect: cli.SideEffectExternalWrite, Positional: cli.PositionalNone,
		Flags: []cli.FlagSpec{
			{Name: "username", Type: cli.FlagTypeString, Required: true, Description: "本地 WebUI 用户名"},
		...
		Run: func(ctx *cli.Context) error {
			...
			if password == "" {
				...
				if err != nil {
					return fmt.Errorf("read webui password: %w", err)
				}
			}
			if err := c.executor.ResetPassword(ctx.Context, ctx.GetString("username"), password); err != nil {
				return err
			}
			_, err := fmt.Fprintln(ctx.Stdout, "webui password reset")
			return err
		},
	}
}
