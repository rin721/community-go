...
import { Button, Field, Surface } from "@webui/ui";
...
export default function LoginPage() {
  ...
  return <Surface className="auth-panel"><div className="auth-panel-heading"><span className="auth-symbol">{t("webui.auth.brandSymbol")}</span><p className="page-eyebrow">{t("webui.auth.brand")}</p><h1>{t("webui.auth.login.title")}</h1><p>{t("webui.auth.login.description")}</p></div><form className="auth-form" onSubmit={(event) => { event.preventDefault(); setError(""); setPending(true); login(username, password).then(completeAuthentication).catch((reason: unknown) => setError(t(loginErrorMessageID(reason)))).finally(() => setPending(false)); }}><Field label={t("webui.auth.login.username")} required autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value)} /><Field label={t("webui.auth.login.password")} required type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} />{error && <p className="form-error">{error}</p>}<Button type="submit" disabled={pending}>{pending ? t("webui.auth.login.pending") : t("webui.auth.login.submit")}</Button></form></Surface>;
}
