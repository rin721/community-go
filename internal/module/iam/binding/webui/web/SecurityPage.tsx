...
export default function SecurityPage() {
  ...
  const saveState = saving ? "pending" : dirty ? "dirty" : "clean";
    ...
    <div className="page-sections">
      <PageSection kicker={t("webui.iam.security.panel.kicker")} title={t("webui.iam.security.panel.title")}>
        <div className="security-layout">
          <form className="form-panel form-panel-bounded" onSubmit={(event) => { event.preventDefault(); if (!canSubmit || saving) return; setSaving(true); setMessage(""); changePassword(current, next).then(() => { setMessage(t("webui.iam.security.changed")); setCurrent(""); setNext(""); setConfirm(""); }).catch(() => setMessage(t("webui.iam.error"))).finally(() => setSaving(false)); }}>
            ...
            <StickyActionBar state={saveState} status={saving ? t("webui.iam.saving") : undefined}><Button type="submit" disabled={!canSubmit || saving} aria-busy={saving}>{t("webui.iam.security.submit")}</Button></StickyActionBar>
          </form>
