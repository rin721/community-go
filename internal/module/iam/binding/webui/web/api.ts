...
export const listAccounts=(query?:string,offset=0,limit=100,extra: { status?: "active" | "disabled"; archived?: boolean; roleId?: string } = {})=>requestJSON<ListResult<Account>>(`/api/v1/iam/accounts?offset=${offset}&limit=${limit}${query?`&query=${encodeURIComponent(query)}`:""}${extra.status?`&status=${extra.status}`:""}${extra.archived!==undefined?`&archived=${extra.archived}`:""}${extra.roleId?`&roleId=${encodeURIComponent(extra.roleId)}`:""}`);
