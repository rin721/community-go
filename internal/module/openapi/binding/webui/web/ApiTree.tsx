...
export function ApiTree({ roots, activeId, collapsed, onToggleCollapsed, onSelect }: {
  ...
  return <aside className={styles.treePanel} data-testid="openapi-tree">
    ...
    <div className={styles.treeScroller}>
      <TreeView nodes={filtered} getChildren={(node) => node.children} getKey={(node) => node.id} getNodeData={(node) => node.kind === "operation" ? { "data-testid": "openapi-tree-leaf" } : {}} selectedId={activeId} onSelect={selectNode} expandAll ariaLabel={t("webui.openapi.tree.label")} renderNode={(node) => node.kind === "operation" && node.row ? <span><MethodBadge method={node.row.method} /><span className={styles.treeLeafLabel}>{node.row.operationId}</span></span> : <span className={styles.treeGroupLabel}><span className={styles.treeGroupName}>{node.label}</span><span className={styles.treeGroupCount}>{treeNodeCount(node)}</span></span>} />
