...
export type Assignment={accountId:string;departmentId?:string;positionIds:string[]};
...
export const listDepartments=()=>requestJSON<ListResult<Department>>("/api/v1/organization/departments?offset=0&limit=100").then((value)=>value.items);
...
export const listPositions=()=>requestJSON<ListResult<Position>>("/api/v1/organization/positions?offset=0&limit=100").then((value)=>value.items);
...
export const replaceAssignment=(accountId:string,departmentId:string,positionIds:string[])=>requestJSON<Assignment>(`/api/v1/organization/accounts/${accountId}/assignment`,{method:"PUT",...json({departmentId:departmentId||undefined,positionIds})});
