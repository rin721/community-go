  ...
  return <div className={`${styles.settingsModule} module-page`}>
      ...
      <PageSection kicker={t("webui.settings.appearance.theme.kicker")} title={t("webui.settings.appearance.theme.title")}>
        <div className="toolbar">
          ...
          <SelectField label={t("webui.settings.appearance.preset")} value={value.preset} onValueChange={(preset) => write({ ...value, preset: preset as ThemeValue["preset"] })} options={presets.map((preset) => ({ value: preset, label: t(`webui.settings.appearance.preset.${preset}`) }))} />
          <SelectField label={t("webui.settings.appearance.density")} value={value.density} onValueChange={(density) => write({ ...value, density: density as ThemeValue["density"] })} options={densities.map((density) => ({ value: density, label: t(`webui.settings.appearance.density.${density}`) }))} />
          <Switch label={t("webui.settings.appearance.reduceMotion")} checked={value.reduceMotion} onChange={(reduceMotion) => write({ ...value, reduceMotion })} />
        </div>
      </PageSection>
      <PageSection kicker={t("webui.settings.appearance.experience.kicker")} title={t("webui.settings.appearance.experience.title")}>
        <div className="toolbar">
          <Switch label={t("webui.settings.appearance.smoothScroll")} checked={experience.smoothScroll} onChange={(smoothScroll) => withExperience({ smoothScroll })} />
          <Switch label={t("webui.settings.appearance.edgeDamping")} checked={experience.edgeDamping} onChange={(edgeDamping) => withExperience({ edgeDamping })} />
          <Switch label={t("webui.settings.appearance.reveal")} checked={experience.reveal} onChange={(reveal) => withExperience({ reveal })} />
          <SelectField label={t("webui.settings.appearance.damping")} value={experience.damping} onValueChange={(damping) => withExperience({ damping: damping as Experience["damping"] })} options={(["subtle", "standard", "relaxed"] as const).map((item) => ({ value: item, label: t(`webui.settings.appearance.damping.${item}`) }))} />
          <SelectField label={t("webui.settings.appearance.revealRhythm")} value={experience.revealRhythm} onValueChange={(revealRhythm) => withExperience({ revealRhythm: revealRhythm as Experience["revealRhythm"] })} options={(["calm", "balanced", "playful"] as const).map((item) => ({ value: item, label: t(`webui.settings.appearance.revealRhythm.${item}`) }))} />
          <SelectField label={t("webui.settings.appearance.scrollbar")} value={experience.scrollbar} onValueChange={(scrollbar) => withExperience({ scrollbar: scrollbar as Experience["scrollbar"] })} options={(["stable", "overlay"] as const).map((item) => ({ value: item, label: t(`webui.settings.appearance.scrollbar.${item}`) }))} />
        </div>
      </PageSection>
    </div>
