# Agent Rules：backend 子树规则

本文件约束 `backend/` 子树内的后台管理 / 控制台平台代码、配置、脚本、文档、测试、构建、部署和运行态资料。聚合仓库根目录 `../AGENTS.md` 是项目级统一入口；本文件只补充 backend 子树内的执行边界。开发文档入口为 `docs/README.md`。

## 产品定位

- `backend/` 是可运行、可扩展、可二次开发的开源后台管理 / 控制台平台底座。
- 主平台统一承载账号、权限、组织租户、配置、审计日志、API catalog、媒体、版本、系统管理、初始化和基础运营能力。
- 业务扩展通过 `internal/modules`、`internal/app/initapp`、HTTP route contract、前端 API client、页面、i18n、测试和文档同步落地。
- 公开站点、首次安装向导、`/admin` 后台、组件体系和质量工具都服务该定位。
- 前端生产能力必须对应后端 API、权限、配置、持久化和审计边界。

## 强制规则

- 交付面只保留当前架构需要的产物、入口、字段、示例、文档、逻辑和运行路径。
- 发现与当前设计不一致的实现后，必须收敛到当前设计并清理关联实现，不允许双轨并存。
- CLI、WebUI、runtime、配置加载和初始化流程共享同一实现来源。
- 修改代码前必须分析现状、调用链、依赖关系和影响边界。
...
- 可变业务策略、品牌标识、产品维度、平台维度、认证安全策略、会话并发策略、Cookie/CSRF/header 名称、缓存开关、缓存 TTL、运行时默认值和部署差异必须进入配置结构、默认配置、示例配置、环境变量覆盖、route contract、受控注册表或系统配置管理。
- 根 README 可以保留项目代号、徽章、Logo 和仓库品牌叙事；运行时代码、配置默认值、API、日志、错误信息、前端生产文案和模块命名使用受控配置或当前模块语境。
...
- 稳定协议值、HTTP 方法、数据库列名、迁移回填值、枚举类型、错误码、编译期 contract 标识和包内私有常量可以保留在代码中，但不承载可运营、可部署、可品牌化或可按产品线变化的策略。
...
- 后端 API 使用 `types/result` 的统一响应结构；handler 返回稳定 JSON envelope。
...
- 前端统一通过所属项目的 API client 和错误归一化逻辑消费请求错误。
...
- 新增或修改后台菜单项时，如果菜单项配置 `permission`，必须同时配置合法 `scope`，并确保该权限能从 route contract 派生的 API catalog 中找到同 `productCode + scope + permission` 的声明。
- 主系统 API handler 使用的请求体 DTO 必须是稳定 Go 类型；普通新增 API 使用明确 DTO。
- `docs/api/openapi.yaml` 是生成产物；变更 route contract 后必须运行 `go run ./cmd/console api openapi --output docs/api/openapi.yaml`。
- `GET /openapi.yaml` 是公开运行时契约接口，不进入 `/api/v1` API catalog、权限同步、操作记录或 SPA fallback。

## 前端与 i18n

- `web/app` 是 React 统一前端，覆盖公开页面、首次安装向导和 `/admin` 后台。
- React 后台 API 统一通过 `app/lib/api` 的 endpoint 表和 API client。
- 首次安装向导位于 `/setup/*`，安装步骤、字段、驱动、选项、测试能力和完成状态来自后端 setup schema 与 status API。
- 用户可见文案维护在 locale 资源中，页面、组件、store、配置、表单 schema、表格列和 SEO helper 使用 locale key。
...
- 后端业务扩展统一落在 `internal/modules`、`internal/app/initapp`、route contract、API catalog、权限、前端 API client、页面、i18n、测试和文档同步链路。
...
- 仓库级平台化重构、最终验收审计、模块化扩展、扩展边界治理、README/AGENTS/docs 同步或发布前 readiness 任务，必须优先使用项目 skill `.agents/skills/aoi-admin-platform-maintenance`。
...
- 模块化扩展边界治理使用 `.agents/skills/aoi-admin-plugin-removal`；React WebUI/i18n 变更使用 `.agents/skills/aoi-admin-webui-i18n`。
...
- 自动提交前必须只暂存本次任务相关文件；若存在无关用户改动、验证失败、密钥/本地配置/运行态数据混入、合并冲突或用户要求不提交，必须停止自动提交并说明原因。
...
- 涉及模块扩展边界、受控配置示例、前端后台入口或生产交付面 API 路径时，必须运行 `scripts/check-plugin-removal.ps1`。
...
- 发布证据应从 `docs/release/release-evidence-template.md` 复制，并通过 `scripts/check-release-evidence.ps1 -Path <发布证据文件>` 校验；未执行项、空占位和明文密钥不能写成通过。
- 当前机器无法运行 Docker 或 Bash 时，必须在目标环境或 CI 使用 `scripts/docker-smoke.ps1` 或 `scripts/docker-smoke.sh` 补证。
