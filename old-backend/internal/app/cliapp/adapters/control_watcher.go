...
func WatchManagedServiceControl(ctx context.Context, service string, controlPath string) (<-chan ControlRequest, error) {
	if os.Getenv(ManagedServiceEnvName) == "" {
		return nil, nil
	}
	out := make(chan ControlRequest, 1)
	service = normalizeServiceName(service)
	self, err := CurrentProcessInfo()
	if err != nil {
		return nil, err
	}
	go func() {
...
func CurrentProcessInfo() (ProcessInfo, error) {
	pid := os.Getpid()
	createTime, err := lookupProcessCreateTime(pid)
	if err != nil {
		return ProcessInfo{PID: pid}, fmt.Errorf("resolve current process create time %d: %w", pid, err)
	}
	return ProcessInfo{PID: pid, ProcessStartTime: createTime}, nil
}
