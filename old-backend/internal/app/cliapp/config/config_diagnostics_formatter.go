...
func printConfigDiagnostics(w interface{ Write([]byte) (int, error) }, configPath string, diagnostics []appconfig.ConfigDiagnostic, localizers ...*localization.Localizer) error {
	if w == nil || len(diagnostics) == 0 {
		return nil
	}
	if _, err := fmt.Fprint(w, formatConfigDiagnostics(configPath, diagnostics, false, firstConfigLocalizer(localizers...))); err != nil {
		return fmt.Errorf("write config diagnostics: %w", err)
	}
	return nil
}
