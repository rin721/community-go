...
func promptDatabasePreflightRepair(ctx *cli.Context, configPath string, cfg *appconfig.Config) (bool, error) {
	...
	case preflightDatabaseActionSQLite:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.database.info.sqlite"), "write database preflight sqlite notice"); err != nil {
			return false, err
		}
		return true, nil
	case preflightDatabaseActionFile:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.database.info.file"), "write database preflight file notice"); err != nil {
			return false, err
		}
		return true, nil
	case preflightActionRuntimeEnvOnly:
		...
		if err := writeConfigPromptInfo(ctx, localizer.T("cli.config.preflight.database.info.runtimeEnv"), "write database preflight runtime env notice"); err != nil {
			return false, err
		}
		return true, nil
