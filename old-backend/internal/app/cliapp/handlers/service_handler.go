...
import (
	...
	"github.com/open-console/console-platform/internal/app/cliapp/output"
...
func (h *ServiceHandler) Execute(ctx *cli.Context) error {
	...
	for {
		...
		case "status":
			...
			if err := output.PrintServiceState(ctx.Stdout, state, localizer); err != nil {
				return err
			}
		case "info":
			state, err := manager.Status(ctx.Context, managed.ServiceServer)
...
func (h *ServiceHandler) Status(ctx *cli.Context) error {
	...
	return output.PrintServiceState(ctx.Stdout, state, localizer)
}
