...
const managedExecutableBaseName = "console-platform-managed"
...
func copyExecutable(source string, target string) error {
	...
	if _, err := io.Copy(out, in); err != nil {
		err = mergeExecutableCopyCloseError(err, out.Close())
		return withExecutableTempCleanup(err, tmp)
	}
	...
	if err := removeManagedExecutableTarget(target); err != nil {
		return withExecutableTempCleanup(err, tmp)
	}
	if err := os.Rename(tmp, target); err != nil {
		return withExecutableTempCleanup(fmt.Errorf("replace managed executable: %w", err), tmp)
	}
	return nil
}

func mergeExecutableCopyCloseError(copyErr, closeErr error) error {
	if closeErr == nil {
		return copyErr
	}
	return errors.Join(copyErr, fmt.Errorf("close temporary executable after copy failure: %w", closeErr))
}

func removeManagedExecutableTarget(path string) error {
	if err := os.Remove(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("remove existing managed executable: %w", err)
	}
	return nil
}

func withExecutableTempCleanup(primary error, tmp string) error {
	if cleanupErr := removeManagedExecutableTemp(tmp); cleanupErr != nil {
		return errors.Join(primary, cleanupErr)
	}
	return primary
}

func removeManagedExecutableTemp(path string) error {
	if err := os.Remove(path); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		return fmt.Errorf("remove temporary managed executable: %w", err)
	}
	return nil
}
