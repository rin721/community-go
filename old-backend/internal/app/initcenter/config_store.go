...
var (
	bootstrapStatePath       = filepath.Join("data", "setup", "bootstrap_state.json")
...
func (s InitConfigStore) Save(ctx context.Context, stepKey string, values map[string]any, persist bool) (ConfigSaveResult, error) {
	...
	if err := s.core.ConfigManager.UpdateWithError(func(cfg *appconfig.Config) error {
...
func clearBootstrapState() error {
	if err := removeBootstrapStateFile(bootstrapStatePath); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("clear setup bootstrap state: %w", err)
	}
	return nil
}
...
func inputFingerprintFor(stepKey string, values map[string]any) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encode setup input fingerprint for %s: %w", stepKey, err)
	}
