...
import (
	...
	storagepkg "github.com/open-console/console-platform/pkg/storage"
)
...
func TestReloadWarnsWhenOptionalComponentCloseFails(t *testing.T) {
	clearReloadEnv(t)
	...
	newCfg.Storage.Driver = config.StorageDriverDisabled
	core, infra, transport, fakes := newReloadFixture(t)
	fakes.cache.closeErr = errors.New("cache close failed")
	fakes.storage.closeErr = errors.New("storage close failed")
	...
	assertWarning(t, fakes.logger, "cache close failed", "disable", fakes.cache.closeErr)
	assertWarning(t, fakes.logger, "storage close failed", "disable", fakes.storage.closeErr)
}
...
// Warn 实现测试日志桩的同名输出入口，用于验证 best-effort 失败仍然可观测。
func (l *fakeLogger) Warn(message string, fields ...interface{}) {
	entry := fakeWarning{
		message: message,
...
func assertWarning(t *testing.T, logger *fakeLogger, message string, action string, wantErr error) {
	t.Helper()
	...
	for _, warning := range logger.warnings {
		...
		if got := warning.fields["action"]; got != action {
			t.Fatalf("expected warning action %q, got %#v", action, got)
		}
		gotErr, ok := warning.fields["error"].(error)
		if !ok {
			t.Fatalf("expected warning error field to be error, got %#v", warning.fields["error"])
		}
		if !errors.Is(gotErr, wantErr) {
			t.Fatalf("expected warning error %v, got %v", wantErr, gotErr)
		}
		return
	}
	t.Fatalf("expected warning %q, got %#v", message, logger.warnings)
}
...
func (e *fakeExecutor) Shutdown() error {
	e.shutdowns++
	return nil
}
