...
const (
	...
	DefaultNotificationRetryIntervalSeconds = 60
	DefaultNotificationRetryBatchSize       = 20
	DefaultNotificationRetryMaxAttempts     = 5
	DefaultLoginMaxFailures                 = 5
	DefaultLoginLockMinutes                 = 15
	DefaultCaptchaTTLSeconds                = 120
	DefaultMFAIssuer                        = "console-platform"
	DefaultNotificationDriver               = "debug"
	DefaultAuthCookieNamePrefix             = "console"
	DefaultAuthCookiePath                   = "/"
	DefaultAuthCookieSameSite               = "lax"
	DefaultAuthCSRFCookieName               = "console_csrf"
	DefaultAuthCSRFHeaderName               = "X-CSRF-Token"
	DefaultAuthProductHeader                = "X-Product-Code"
	DefaultAuthClientTypeHeader             = "X-Client-Type"
...
type AuthConfig struct {
	...
	NotificationRetryIntervalSeconds int                  `mapstructure:"notification_retry_interval_seconds" envname:"AUTH_NOTIFICATION_RETRY_INTERVAL_SECONDS" json:"notification_retry_interval_seconds" yaml:"notification_retry_interval_seconds" toml:"notification_retry_interval_seconds"`
	NotificationRetryBatchSize       int                  `mapstructure:"notification_retry_batch_size" envname:"AUTH_NOTIFICATION_RETRY_BATCH_SIZE" json:"notification_retry_batch_size" yaml:"notification_retry_batch_size" toml:"notification_retry_batch_size"`
	NotificationRetryMaxAttempts     int                  `mapstructure:"notification_retry_max_attempts" envname:"AUTH_NOTIFICATION_RETRY_MAX_ATTEMPTS" json:"notification_retry_max_attempts" yaml:"notification_retry_max_attempts" toml:"notification_retry_max_attempts"`
...
func (c *AuthConfig) Validate() error {
	...
	if c.CasbinReloadIntervalSeconds <= 0 {
		return fmt.Errorf("casbin_reload_interval_seconds must be positive")
	}
	if c.NotificationRetryIntervalSeconds <= 0 || c.NotificationRetryBatchSize <= 0 || c.NotificationRetryMaxAttempts <= 0 {
		return fmt.Errorf("notification retry interval, batch size, and max attempts must be positive")
	}
...
func (c *AuthConfig) ApplyDefaults() {
	...
	if c.Issuer == "" {
		c.Issuer = "console-platform"
	}
	if len(c.Audience) == 0 {
		c.Audience = []string{"console-platform-api"}
	}
	...
	if c.NotificationRetryIntervalSeconds == 0 {
		c.NotificationRetryIntervalSeconds = DefaultNotificationRetryIntervalSeconds
	}
	if c.NotificationRetryBatchSize == 0 {
		c.NotificationRetryBatchSize = DefaultNotificationRetryBatchSize
	}
	if c.NotificationRetryMaxAttempts == 0 {
		c.NotificationRetryMaxAttempts = DefaultNotificationRetryMaxAttempts
	}
	if strings.TrimSpace(c.Cookie.NamePrefix) == "" {
