...
func TestSMTPNotifierRendersInvitationPasswordResetAndEmailVerification(t *testing.T) {
	...
	notifier, err := NewSMTPNotifier(SMTPNotifierConfig{
		Sender:       sender,
		TemplateData: map[string]any{"ProductName": "Console Platform"},
	...
	if got := sender.last; got.Subject != "Invite to Console Platform" || got.TextBody != "Join Console Platform at https://example.com/invite" {
	...
	if got := sender.last; got.Subject != "Reset Console Platform password" || got.TextBody != "Reset at https://example.com/reset" {
	...
	if got := sender.last; got.Subject != "Verify Console Platform email" || got.TextBody != "Verify at https://example.com/verify" {
