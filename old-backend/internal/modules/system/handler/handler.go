...
import (
	...
	"github.com/open-console/console-platform/internal/modules/system/service"
...
func (h *Handler) StreamTrafficHijack(c ports.HTTPContext) {
	...
	defer cancel()
	if err := writeSSE(writer, flusher, "ready", map[string]string{"status": "ok"}); err != nil {
		h.warnStreamWrite(err, "ready")
		return
	}
	for {
		select {
		...
		case event, ok := <-events:
			...
			if err := writeSSE(writer, flusher, event.Type, event.Payload); err != nil {
				h.warnStreamWrite(err, event.Type)
				return
			}
		}
	}
...
func (h *Handler) filterMenus(ctx context.Context, principal authtypes.Principal, groups []model.MenuGroup) []model.MenuGroup {
...
func (h *Handler) allowed(ctx context.Context, principal authtypes.Principal, item model.MenuItem) bool {
	if h.authorizer == nil {
		return false
	}
	...
	allowed, err := h.authorizer.Authorize(ctx, principal, authtypes.PermissionContext{
...
func (h *Handler) warnStreamWrite(err error, event string) {
	if h.logger != nil {
		h.logger.Warn("system traffic hijack stream write failed", "event", event, "error", err)
	}
}
