...
func (r *repository) ListMediaUploadSessionsForCleanup(ctx context.Context, now time.Time, limit int) ([]model.MediaUploadSession, error) {
	if limit <= 0 {
		return nil, nil
	}
	var sessions []model.MediaUploadSession
	err := r.db.Find(ctx, &sessions,
		alive(),
		database.Where("id IN (SELECT DISTINCT session_id FROM system_media_upload_chunks)"),
		database.Where(
			"(status IN ? OR (status = ? AND expires_at <= ?))",
			[]string{model.MediaUploadStatusAborted, model.MediaUploadStatusCompleted, model.MediaUploadStatusExpired},
			model.MediaUploadStatusActive,
			now,
		),
		database.Order("updated_at ASC, id ASC"),
