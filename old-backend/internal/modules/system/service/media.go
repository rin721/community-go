...
func (s *service) UploadMediaAsset(ctx context.Context, input UploadMediaAssetInput) (*model.MediaAsset, error) {
	...
	if err := s.repo.CreateMediaAsset(ctx, asset); err != nil {
		s.rollbackMediaAssetObject(key, "upload")
		if isStorageUnavailable(err) {
			return nil, ErrStorageUnavailable
		}
		return nil, err
...
// rollbackMediaAssetObject 在媒体资产元数据落库失败后尽力删除已写入的正式对象。
// 主错误仍返回给调用方；删除失败只记录告警，方便后续通过对象存储巡检或人工补偿清理孤立文件。
func (s *service) rollbackMediaAssetObject(key string, source string) {
	if s.objectStore == nil || strings.TrimSpace(key) == "" {
		return
	}
	err := s.objectStore.Remove(key)
	if err == nil || errors.Is(err, os.ErrNotExist) || s.logger == nil {
		return
	}
	s.logger.Warn("media asset object rollback failed", "storage_key", key, "source", source, "error", err)
}
