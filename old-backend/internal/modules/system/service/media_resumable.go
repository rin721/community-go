...
func (s *service) CompleteMediaResumableUpload(ctx context.Context, input CompleteMediaResumableUploadInput) (model.MediaResumableCompleteResult, error) {
	...
	for expected, chunk := range chunks {
		...
		if err := appendMediaUploadChunk(&merged, chunk.ChunkIndex, data); err != nil {
			return result, err
		}
	}
	data := merged.Bytes()
	...
	if err := s.repo.CreateMediaAsset(ctx, asset); err != nil {
		...
		s.rollbackMediaAssetObject(key, "resumable")
		return result, mapMediaStorageError(err)
	}
...
func appendMediaUploadChunk(w io.Writer, chunkIndex int, data []byte) error {
	if _, err := w.Write(data); err != nil {
		return fmt.Errorf("%w: merge media upload chunk %d: %w", ErrStorageUnavailable, chunkIndex, err)
	}
	return nil
}
...
func (s *service) cleanupMediaUploadChunks(ctx context.Context, sessionID int64) {
	if s.objectStore != nil {
		if err := s.objectStore.RemoveAll(mediaChunkStorageDir(s.cfg.MediaPrefix, sessionID)); err != nil {
			s.warnMediaCleanupError("media upload chunk file cleanup failed", sessionID, err)
		}
	}
	if s.repo != nil {
		if err := s.repo.DeleteMediaUploadChunks(ctx, sessionID); err != nil {
			s.warnMediaCleanupError("media upload chunk record cleanup failed", sessionID, err)
		}
	}
}

func (s *service) warnMediaCleanupError(message string, sessionID int64, err error) {
	if s.logger == nil || err == nil {
		return
	}
	s.logger.Warn(message, "session_id", sessionID, "error", err)
}
