...
- `.env` 加载通过 `configloader.LoadEnv(path)` 触发,调用方不直接依赖 godotenv；读取或解析错误必须返回给上层,由上层决定缺失文件是否可跳过。
- `UpdateYAMLScalars` 会保留配置文件身份并原地写入；写入失败时会尝试恢复旧内容。主写入错误、恢复失败、临时文件关闭失败和临时文件清理失败都会返回给上层,调用方必须把它视为需要提示或人工修复的配置持久化失败。
- 如果需要新增配置能力,优先扩展本包方法,不要让第三方配置库类型进入 `internal/`。

## 基本使用

```go
if err := configloader.LoadEnv(".env"); err != nil {
    return err
}
