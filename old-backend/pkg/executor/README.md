...
func main() {
    ...
    defer func() {
        if err := mgr.Shutdown(); err != nil {
            log.Printf("executor shutdown failed: %v", err)
        }
    }()
...
| `Shutdown() error`               | 优雅关闭,等待任务完成并返回释放错误 |
...
3. ✅ 获取写锁 → 原子替换 → 释放写锁
4. ✅ 在锁外优雅关闭旧池
5. ✅ 新池创建失败时保持原配置不变；旧池释放失败时返回清理错误并保留新配置已经生效的事实
...
- `nil`: 所有池已释放
- 非空错误: 至少一个池释放超时或释放失败；错误会包含池名称，调用方应记录并上抛到生命周期关闭结果
...
1. ✅ 新任务提交到新池
2. ✅ 旧池等待任务完成 (最多 5 秒)
3. ✅ 旧池最多等待 5 秒；超时会返回关闭错误，调用方据此记录或上抛
