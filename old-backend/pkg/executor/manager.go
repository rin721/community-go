...
//	        log.Error("failed to shutdown executor", "error", err)
//	    }
//	}()
func NewManager(configs []Config) (Manager, error) {
	...
	for _, cfg := range configs {
		...
		if _, exists := pools[cfg.Name]; exists {
			// 清理已创建的池
			cleanupErr := releasePools(pools)
			return nil, errors.Join(
				fmt.Errorf("%w: duplicate pool name: %s", ErrInvalidConfig, cfg.Name),
				wrapReleasePoolsError("cleanup partially created executor pools", cleanupErr),
			)
		}
		...
		if err != nil {
			...
			cleanupErr := releasePools(pools)
			return nil, errors.Join(
				fmt.Errorf("failed to create pool %s: %w", cfg.Name, err),
				wrapReleasePoolsError("cleanup partially created executor pools", cleanupErr),
			)
		}
...
func (m *manager) Reload(configs []Config) error {
	...
	for _, cfg := range configs {
		...
		if _, exists := newPools[cfg.Name]; exists {
			cleanupErr := releasePools(newPools)
			return errors.Join(
				fmt.Errorf("%w: duplicate pool name: %s", ErrInvalidConfig, cfg.Name),
				wrapReleasePoolsError("cleanup partially created executor pools", cleanupErr),
			)
		...
		if err != nil {
			...
			cleanupErr := releasePools(newPools)
			return errors.Join(
				fmt.Errorf(ErrMsgReloadFailed, err),
				wrapReleasePoolsError("cleanup partially created executor pools", cleanupErr),
			)
		}
	...
	if err := releasePools(oldPools); err != nil {
		return fmt.Errorf("release old executor pools after reload: %w", err)
	}
...
func (m *manager) Shutdown() error {
	...
	// 在锁外释放池
	if err := releasePools(pools); err != nil {
		return fmt.Errorf("shutdown executor pools: %w", err)
	}
	return nil
}
...
//
//	pools: 要释放的池 map
func releasePools(pools map[PoolName]*poolWrapper) error {
	return releasePoolsWithTimeout(pools, ShutdownTimeout)
}

func releasePoolsWithTimeout(pools map[PoolName]*poolWrapper, timeout time.Duration) error {
	if len(pools) == 0 {
		return nil
	}
	...
	for name, pool := range pools {
		...
		go func() {
			...
			if err := pool.ReleaseTimeout(timeout); err != nil {
				errCh <- fmt.Errorf("release pool %s: %w", name, err)
			}
		}()
	}
...
func wrapReleasePoolsError(action string, err error) error {
	if err == nil {
		return nil
	}
