...
4. ✅ **失败保护**: 新配置创建失败时保持原配置不变；旧 logger 同步失败会返回“重载已应用但旧 logger 同步失败”的错误状态
5. ✅ **线程安全**: 使用读写锁保护,并发安全
...
### 问：如何处理 stdout 的 sync 错误？

```go
// 本包会把 stdout/stderr 在部分平台上的无害 EINVAL 归一化为 nil。
// 调用方只需要处理剩余的真实 sync 错误。
if err := log.Sync(); err != nil {
    log.Error("failed to sync logger", "error", err)
    return err
}
