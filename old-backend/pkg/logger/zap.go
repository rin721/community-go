...
// 使用场景:
//
//	if err := logger.Sync(); err != nil { ... } // 在 main 函数中
//	logger.Sync()                               // 在优雅关闭时
//
// 返回:
//
//	error: 刷新失败时的错误。stdout/stderr 在部分平台上的 EINVAL 会被归一化为 nil。
func (l *zapLogger) Sync() error {
	l.mu.RLock()
	sugar := l.sugar
	l.mu.RUnlock()

	return normalizeSyncError(sugar.Sync())
}
...
// 并发安全:
//   - 使用写锁保护,确保重载过程原子性
//   - 新 logger 创建失败时保持原有 logger 不变
//   - 新 logger 创建成功后才替换旧 logger
//   - 旧 logger 同步失败时返回“重载已应用但旧 logger 同步失败”的错误状态
func (l *zapLogger) Reload(cfg *Config) error {
	...
	if oldSugar != nil {
		if err := normalizeSyncError(oldSugar.Sync()); err != nil {
			return fmt.Errorf("logger reload applied but old logger sync failed: %w", err)
		}
	}
...
func normalizeSyncError(err error) error {
	...
	// os.Stdout/os.Stderr 在部分平台或运行容器中不支持 fsync，zap 会返回
	// EINVAL。该错误不代表日志写入失败；真实文件、轮转器或其他 sync 错误仍返回上层。
