...
func (r *runner) run(ctx context.Context, w io.Writer, fn func(context.Context) error) error {
	...
	logger := &writerLogger{w: writer}
	goose.SetLogger(logger)
	return errors.Join(fn(ctx), logger.Err())
}
...
func (l *writerLogger) write(format string, v ...interface{}) {
	if _, err := fmt.Fprintf(l.w, format+"\n", v...); err != nil {
		l.mu.Lock()
		defer l.mu.Unlock()
		l.err = errors.Join(l.err, fmt.Errorf("write migration output: %w", err))
	}
}

func (l *writerLogger) Err() error {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.err
}
