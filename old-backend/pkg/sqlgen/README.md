...
```go
import "github.com/open-console/console-platform/pkg/sqlgen"
```
...
| `Migrate(model).Build()` | `model` 解析失败会在 `Build()` 返回错误，不生成空表名 `ALTER TABLE` | [CONFIRMED] guarded | 调用方必须检查 `Build()` 返回的 error |
