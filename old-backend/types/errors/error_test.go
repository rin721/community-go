...
func TestGlobalErrorCodesStayPlatformLevel(t *testing.T) {
	content, err := os.ReadFile("codes.go")
	if err != nil {
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(source, forbidden) {
			t.Fatalf("types/errors must not expose module-specific error code %s", forbidden)
		}
	}
}
