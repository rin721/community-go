...
import { adminNavGroupsFromSystemMenus } from "~/features/admin/navigation";
...
function renderAdminHeaderWithGroups(pathname = "/admin/users") {
  const queryClient = new QueryClient({
    defaultOptions: { queries: { retry: false } },
  });
  const groups = adminNavGroupsFromSystemMenus([
    {
      code: "workspace",
      items: [
        {
  ...
  return render(
    <I18nextProvider i18n={i18n}>
      <QueryClientProvider client={queryClient}>
        <MemoryRouter initialEntries={[pathname]}>
          <AdminHeader groups={groups} pathname={pathname} />
        </MemoryRouter>
      </QueryClientProvider>
    </I18nextProvider>,
  );
...
describe("AdminHeader", () => {
  ...
  it("hides persisted workspace tabs that are not present in backend-filtered navigation", async () => {
    useAdminWorkspaceStore.setState({
      hydrated: true,
      tabs: [
        { fixed: true, id: "dashboard", labelKey: "admin.nav.dashboard", to: "/admin" },
        { id: "users", labelKey: "admin.nav.users", to: "/admin/users" },
      ],
    });

    renderAdminHeaderWithGroups("/admin");

    expect(await screen.findByRole("link", { name: "Dashboard" })).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: en.admin.nav.users })).toBeNull();
  });
