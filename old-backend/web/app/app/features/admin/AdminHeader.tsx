...
import { IconButton } from "~/components/console/primitives/IconButton";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  DropdownMenuTrigger,
} from "~/components/console/patterns/DropdownMenu";
...
export function AdminHeader({ groups, pathname }: AdminHeaderProps) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [busyAction, setBusyAction] = useState<HeaderAction | null>(null);
  const [statusMessage, setStatusMessage] = useState("");
  const currentGroup = useMemo(() => findAdminNavGroup(pathname, groups), [groups, pathname]);
  const currentItem = useMemo(() => findAdminNavItem(pathname, groups), [groups, pathname]);
  ...
  const visibleTabs = useMemo(
    () => tabs.filter((tab) => tab.fixed || hasAdminNavItemForPath(tab.to, groups)),
    [groups, tabs],
  );
  ...
  function handleCloseTab(tab: AdminWorkspaceTab) {
    const remainingTabs = visibleTabs.filter(
      (candidate) => candidate.fixed || candidate.to !== tab.to,
    );
    closeTab(tab.to);
  ...
  return (
    <header className="console-admin-header">
      <div className="console-admin-topbar">
        <div className="console-admin-topbar__context">
          <span>{adminNavLabel(currentGroup, t)}</span>
          <strong>{adminNavLabel(currentItem, t)}</strong>
        </div>
        <div className="console-admin-topbar__actions">
          <OrganizationMenu
      ...
      <nav className="console-admin-tabs" aria-label={t("admin.header.tabs.label")}>
        {visibleTabs.map((tab) => {
          const active = samePath(tab.to, pathname);
          const item = findAdminNavItem(tab.to, groups);
          const TabIcon = item.icon;

          return (
            <span
              className="console-admin-tab"
              data-active={active ? "true" : "false"}
              key={tab.to}
            >
              <Link to={tab.to}>
                <TabIcon aria-hidden="true" size={15} />
                <span>{item.label || t(tab.labelKey)}</span>
              </Link>
              {tab.fixed ? null : (
                <IconButton
                  appearance="ghost"
                  className="console-admin-tab__close"
      ...
      {statusMessage ? (
        <p className="console-admin-header__status" role="status">
          {statusMessage}
        </p>
      ) : null}
    </header>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="console-admin-org-trigger"
          type="button"
        >
          <span>
      ...
      <DropdownMenuContent className="console-admin-org-menu" align="end">
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="console-admin-user-trigger"
          type="button"
        >
          <span className="console-admin-user-trigger__avatar" aria-hidden="true">
            {name.slice(0, 1).toUpperCase()}
          </span>
          <span className="console-admin-user-trigger__copy">
            <strong>{name}</strong>
      ...
      <DropdownMenuContent className="console-admin-user-menu" align="end">
        <DropdownMenuLabel>{t("admin.header.account")}</DropdownMenuLabel>
        <div className="console-dropdown-menu-profile">
...
function sameId(left: unknown, right: unknown) {
  const leftID = comparableID(left);
  const rightID = comparableID(right);
  return leftID !== undefined && rightID !== undefined && leftID === rightID;
}

function comparableID(value: unknown) {
