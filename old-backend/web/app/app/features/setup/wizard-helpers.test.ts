...
describe("setup wizard helpers", () => {
  ...
  it("does not hydrate sensitive backend values into browser form state", () => {
    ...
    expect(hydrateValues({}, schema, "en-US")).toEqual({
      "database.configure": {
    ...
    expect(
      hydrateValues({ "database.configure": { "database.password": "entered" } }, schema, "en-US"),
    ).toMatchObject({
      "database.configure": {
  ...
  it("keeps setup default locale fields on canonical locale codes", () => {
    const schema: SetupSchema = {
      steps: [
    ...
    expect(hydrateValues({}, schema, "en-US")).toMatchObject({
      "system.configure": {
  ...
  it("requires matching owner password confirmation without backend schema changes", () => {
    ...
    expect(
      validateStepFields(ownerStep, {
        password: "Console-password-123!",
        passwordConfirm: "Console-password-123!",
      }),
    ).toBeNull();
  });
