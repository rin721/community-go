...
import builtinThemePackage from "~/theme/packages/builtin/default/theme.json";
import customThemePackage from "~/theme/packages/example/custom/theme.json";
import { consoleThemePackageSchema, requiredThemeTemplateKeys } from "~/theme/schema";
...
describe("theme settings helpers", () => {
  test("validates builtin and example source theme packages", () => {
    expect(consoleThemePackageSchema.safeParse(builtinThemePackage).success).toBe(true);
    expect(consoleThemePackageSchema.safeParse(customThemePackage).success).toBe(true);
  });
  ...
  test("rejects remote theme asset URLs", () => {
    const result = consoleThemePackageSchema.safeParse({
      ...builtinThemePackage,
      assets: {
