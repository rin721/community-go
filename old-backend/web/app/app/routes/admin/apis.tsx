...
const apiCatalogErrorCopy = {
  defaultTitle: "admin.apis.states.errorTitle",
  permissionDescription: "admin.apis.states.permissionDescription",
  permissionTitle: "admin.apis.states.permissionTitle",
};
...
export default function AdminAPIsRoute() {
  ...
  const syncRoutesMutation = useMutation({
    ...
    onError: (error) => {
      setSyncNotice({
        description: adminErrorDescription(error, t, apiCatalogErrorCopy),
        intent: "danger",
    ...
    () => [
      {
        accessorKey: "method",
        cell: ({ getValue }) => (
          <span className="console-api-method" data-method={String(getValue()).toLowerCase()}>
      ...
      },
      {
        accessorKey: "path",
        cell: ({ row }) => (
          <div className="console-api-path">
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="console-api-group">
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className="console-api-access" data-access={access}>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="console-api-permission">
      ...
      },
      {
        id: "registration",
        cell: ({ row }) => (
          <span className="console-api-status" data-status={registrationStatus(row.original)}>
      ...
      },
      {
        accessorKey: "synced",
        cell: ({ row }) => (
          <div className="console-api-sync">
            <span
              className="console-api-status"
  ...
  const canSyncAPICatalog = hasSessionPermission(permissions, {
  ...
  const confirmSync = () => {
    if (!canSyncAPICatalog) {
      setPendingSyncAction(null);
      setSyncNotice({
        description: t("admin.apis.actions.syncDisabled"),
        intent: "danger",
        title: t("admin.apis.states.permissionTitle"),
      });
      return;
    }
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-apis-title">
      <div className="console-admin-page-header">
        ...
        <div className="console-admin-action-row">
          <Button
          ...
          <Button
            appearance="secondary"
            disabled={syncBusy || !canSyncAPICatalog}
            icon={<RefreshCw size={17} />}
            onClick={() => setPendingSyncAction("routes")}
            title={canSyncAPICatalog ? undefined : t("admin.apis.actions.syncDisabled")}
          >
      ...
      {apiCatalogQuery.error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(apiCatalogQuery.error, t, apiCatalogErrorCopy)}
          description={adminErrorDescription(apiCatalogQuery.error, t, apiCatalogErrorCopy)}
        />
      ) : null}
      {syncNotice ? (
        <StateBlock
      ...
      {pendingSyncAction ? (
        <StateBlock
          action={
            <div className="console-api-confirm-actions">
              <Button
                disabled={!canSyncAPICatalog}
                loading={syncBusy}
                title={canSyncAPICatalog ? undefined : t("admin.apis.actions.syncDisabled")}
                onClick={confirmSync}
              >
      ...
      <div className="console-admin-stat-grid" aria-label={t("admin.apis.summaryLabel")}>
        <APIStatCard
          icon={<Code2 size={19} />}
      ...
      <section className="console-admin-panel">
        ...
        <form className="console-admin-filter-form" onSubmit={(event) => event.preventDefault()}>
          <FormField
      ...
      <section className="console-admin-panel">
        ...
        {apiCatalogQuery.isLoading ? (
          ...
          <div className="console-api-table">
            <DataTable
              columns={apiColumns}
              data={filteredRows}
