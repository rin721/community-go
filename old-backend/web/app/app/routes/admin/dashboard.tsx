...
const dashboardErrorCopy = {
  defaultTitle: "admin.dashboard.states.errorTitle",
  permissionDescription: "admin.dashboard.states.permissionDescription",
  permissionTitle: "admin.dashboard.states.permissionTitle",
};
...
export default function AdminDashboardRoute() {
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-dashboard-title">
      ...
      {loading ? (
        ...
        <div className="console-admin-stat-grid" aria-label={t("admin.dashboard.summaryLabel")}>
          <MetricCard
      ...
      <div className="console-admin-panel-grid console-admin-panel-grid--dashboard">
        <DashboardPanel
          className="console-admin-panel--span-2"
        ...
        >
          {apiCatalogQuery.data ? (
            <>
              <div className="console-admin-inline-stats">
                <span>
...
function MetricCard({ icon, label, state, value }: MetricCardProps) {
  return (
    <article className="console-admin-stat-card" data-state={state}>
...
function DashboardPanel({
  ...
  return (
    <article className={["console-admin-panel", className].filter(Boolean).join(" ")}>
      <header>
        <h2>{title}</h2>
        <p>{description}</p>
      </header>
      {error ? (
        <StateBlock
          intent="danger"
          title={adminErrorTitle(error, t, dashboardErrorCopy)}
          description={adminErrorDescription(error, t, dashboardErrorCopy)}
        />
      ) : loading ? (
        <PanelSkeleton />
      ) : (
...
function ServerResourceOverview({ info }: { info: SystemServerInfo }) {
  ...
  return (
    <div className="console-server-overview">
      <div
        className="console-server-gauge-grid"
      ...
      <dl className="console-server-quick-facts">
        <div>
          <dt>
...
function ResourceGauge({
  ...
  return (
    <article className="console-server-gauge-card">
      <EChart ariaLabel={ariaLabel} className="console-server-gauge-chart" option={option} />
      <div className="console-server-gauge-card__copy">
...
function ServerMetricsHistoryPanel({ history }: { history: SystemServerMetricsHistory }) {
  ...
  const activeDisk =
    selectedDisk !== allDiskValue && diskNames.includes(selectedDisk) ? selectedDisk : allDiskValue;
  const latestDisk = latest ? diskMetricForSample(latest, activeDisk) : undefined;
  ...
  const diskOption = useMemo(
    () => diskIOHistoryOption(samples, activeDisk, palette, i18n.language, t),
    [activeDisk, i18n.language, palette, samples, t],
  );
  ...
  return (
    <div className="console-server-monitor-panel">
      <div className="console-server-monitor-toolbar">
        <div
          className="console-server-monitor-tabs"
        ...
        {mode === "disk" ? (
          <label className="console-server-monitor-select">
      ...
      {mode === "disk" && diskNames.length === 0 ? (
        ...
        <EChart
          ...
          className="console-server-monitor-chart"
...
function TrafficHijackOverviewPanel({ overview }: { overview: SystemTrafficHijackOverview }) {
  ...
  return (
    <div className="console-traffic-dashboard-panel">
      <div className="console-admin-inline-stats">
        <span>
      ...
      {overview.recentResults.length > 0 ? (
        <EChart
          ...
          className="console-traffic-dashboard-chart"
          option={option}
        />
      ) : (
        <StateBlock
...
function KeyValueList({
  ...
  return (
    <dl className="console-admin-key-values" data-columns={columns}>
...
function resourceGaugeOption(value: number, color: string, trackColor: string): ConsoleChartOption {
