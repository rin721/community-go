...
export default function AdminMediaResumableRoute() {
  ...
  const canUpload = Boolean(
    canUploadMedia && selectedFile && checkResult && !uploadUnavailable && !busy,
  );
  ...
  const uploadPermissionDescription = () =>
    t("admin.mediaResumable.states.writePermissionDescription", {
      permission: "media:upload",
    });

  const setUploadPermissionNotice = () => {
    setNotice({
      description: uploadPermissionDescription(),
      intent: "danger",
      title: t("admin.mediaResumable.states.permissionTitle"),
    });
  };

  const openFilePicker = () => {
    if (!canUploadMedia || busy) {
      if (!canUploadMedia) {
        setUploadPermissionNotice();
      }
      return;
    }
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-media-resumable-title">
      ...
      <div className="console-admin-stat-grid" aria-label={t("admin.mediaResumable.summaryLabel")}>
        <MediaUploadStatCard
      ...
      <section className="console-admin-panel console-media-resumable-panel">
        <header>
          <h2>{t("admin.mediaResumable.upload.title")}</h2>
          <p>{t("admin.mediaResumable.upload.description")}</p>
        </header>

        <div className="console-media-resumable-workbench">
          <div className="console-media-upload-dropzone" data-state={status}>
            <input
              ref={fileInputRef}
            ...
            />
            <span className="console-media-upload-dropzone__icon" aria-hidden="true">
            ...
            <div className="console-media-upload-actions">
              <Button
                appearance="secondary"
                disabled={!canUploadMedia || busy}
                icon={<UploadCloud size={17} />}
                onClick={openFilePicker}
                title={canUploadMedia ? undefined : uploadPermissionDescription()}
              >
          ...
          <div className="console-media-upload-side">
            <SelectField
              disabled={!canUploadMedia || Boolean(selectedFile) || busy}
            ...
            <dl className="console-admin-key-values" data-columns="2">
              <KeyValue
      ...
      {lastAsset ? (
        <section className="console-admin-panel">
          ...
          <dl className="console-admin-key-values" data-columns="3">
            <KeyValue
...
function ProgressMeter({
  ...
  return (
    <div className="console-progress-meter">
      <div>
        <span>{label}</span>
