...
const securityErrorCopy = {
  defaultTitle: "admin.security.states.identityFailedTitle",
};
...
export default function AdminSecurityRoute() {
  ...
  const setupMFAMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setNotice({
        description: adminErrorDescription(error, t, securityErrorCopy),
        intent: "danger",
  ...
  return (
    <section className="console-admin-dashboard" aria-labelledby="admin-security-title">
      ...
      {identityQuery.error ? (
        <StateBlock
          description={adminErrorDescription(identityQuery.error, t, securityErrorCopy)}
          intent="danger"
          title={t("admin.security.states.identityFailedTitle")}
        />
      ) : null}

      <div className="console-security-grid">
        <section className="console-admin-panel">
          <header className="console-security-panel-header">
            <div>
              <h2>{t("admin.security.accountTitle")}</h2>
              <p>{user?.username || t("common.labels.none")}</p>
            </div>
            <span className="console-iam-status" data-status={mfaEnabled ? "active" : "pending"}>
              {t(
          ...
          <SecurityKeyValueList
            ...
            items={accountItems}
          />
          <div className="console-security-actions">
            <Button asChild appearance="secondary">
              <Link to="/admin/sessions">
                <MonitorCheck aria-hidden="true" size={17} />
        ...
        <section className="console-admin-panel">
          ...
          {setupPayload ? (
            <div className="console-mfa-setup">
              <ReadonlySecretField
              ...
              />
              <ConsoleForm form={form} onSubmit={submitMFA}>
                <ConsoleTextField<MFAFormValues>
...
function SecurityKeyValueList({ ariaLabel, items }: { ariaLabel: string; items: SecurityItem[] }) {
  return (
    <dl className="console-security-key-values" aria-label={ariaLabel}>
      {items.map((item) => (
        <div key={item.label}>
          <dt>
...
function ReadonlySecretField({
  ...
  return (
    <div className="console-security-secret-field">
