...
import { EChart } from "~/components/console/patterns/EChart";
import { PanelSkeleton, StatGridSkeleton } from "~/components/console/patterns/LoadingSkeletons";
...
const emptyTrafficEvents: SystemTrafficHijackEvent[] = [];
const emptyTrafficResults: SystemTrafficProbeResult[] = [];
const emptyTrafficTargets: SystemTrafficProbeTarget[] = [];
...
export default function AdminTrafficHijackRoute() {
  ...
  const canUpdateTraffic = hasSessionPermission(permissions, {
    code: "traffic_hijack:update",
    productCode: productCode || undefined,
    scope: "platform",
  });
  const canDeleteTraffic = hasSessionPermission(permissions, {
    code: "traffic_hijack:delete",
    productCode: productCode || undefined,
    scope: "platform",
  });
  const trafficPermissionDescription = useCallback(
    (permission: string) =>
      t("admin.trafficHijack.states.writePermissionDescription", {
        permission,
      }),
    [t],
  );
  const setTrafficPermissionNotice = useCallback(
    (permission: string) => {
      setPermissionNotice({
        description: trafficPermissionDescription(permission),
        title: t("admin.trafficHijack.states.permissionTitle"),
      });
    },
    [t, trafficPermissionDescription],
  );
  ...
  const targets = targetsQuery.data ?? overviewQuery.data?.targets ?? emptyTrafficTargets;
  const results =
    resultsQuery.data?.items ?? overviewQuery.data?.recentResults ?? emptyTrafficResults;
  const events = eventsQuery.data?.items ?? overviewQuery.data?.recentEvents ?? emptyTrafficEvents;

  const submitTarget = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!canUpdateTraffic) {
      setTrafficPermissionNotice("traffic_hijack:update");
      return;
    }
  ...
  const beginEdit = useCallback(
    (target: SystemTrafficProbeTarget) => {
      if (!canUpdateTraffic) {
        setTrafficPermissionNotice("traffic_hijack:update");
        return;
      }
    ...
    [canUpdateTraffic, setTrafficPermissionNotice],
  );
  ...
  const targetColumns = useMemo<ColumnDef<SystemTrafficProbeTarget>[]>(
    () => [
      {
        accessorKey: "name",
        cell: ({ row }) => (
          <div className="console-traffic-target-cell">
      ...
      },
      {
        ...
        cell: ({ getValue }) => formatDate(tableString(getValue()), i18nInstance.language, t),
        header: t("admin.trafficHijack.targets.columns.lastChecked"),
      },
      {
        id: "actions",
        cell: ({ row }) => (
          <div className="console-traffic-row-actions">
            <Button
              appearance="ghost"
              disabled={!canUpdateTraffic}
              icon={<Play size={15} />}
              loading={probeMutation.isPending}
              title={
                canUpdateTraffic ? undefined : trafficPermissionDescription("traffic_hijack:update")
              }
              onClick={() => {
                if (!canUpdateTraffic) {
                  setTrafficPermissionNotice("traffic_hijack:update");
                ...
                probeMutation.mutate(row.original.id);
              }}
            >
            ...
            <Button
              appearance="ghost"
              disabled={!canDeleteTraffic}
              icon={<Trash2 size={15} />}
              loading={deleteMutation.isPending}
              title={
                canDeleteTraffic ? undefined : trafficPermissionDescription("traffic_hijack:delete")
              }
              onClick={() => {
                if (!canDeleteTraffic) {
                  setTrafficPermissionNotice("traffic_hijack:delete");
                  return;
                }
    ...
    [
      ...
      probeMutation,
      setTrafficPermissionNotice,
      t,
      trafficPermissionDescription,
    ],
  );
  ...
  const resultColumns = useMemo<ColumnDef<SystemTrafficProbeResult>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => tableString(getValue(), "-"),
  ...
  return (
    <section className="console-traffic-page" aria-labelledby="admin-traffic-title">
      ...
      {permissionNotice ? (
        <StateBlock
          description={permissionNotice.description}
          intent="danger"
          title={permissionNotice.title}
        />
      ) : null}

      <div className="console-traffic-workbench">
        <article className="console-admin-panel">
          <header>
          ...
          <TargetForm
            ...
            permissionTitle={trafficPermissionDescription("traffic_hijack:update")}
            onCancel={cancelEdit}
            onChange={setForm}
        ...
        <article className="console-admin-panel console-admin-panel--span-2">
        ...
        <article className="console-admin-panel console-admin-panel--span-2">
          ...
          {results.length > 0 ? (
            <EChart
              ...
              className="console-traffic-latency-chart"
        ...
        <article className="console-admin-panel console-admin-panel--span-2">
          ...
          <div className="console-traffic-toolbar">
            <select
...
function TargetForm({
  ...
  return (
    <form className="console-traffic-form" onSubmit={onSubmit}>
      ...
      <div className="console-traffic-form-grid">
        <label className="console-form-field">
      ...
      <label className="console-traffic-check">
        <input
      ...
      <fieldset className="console-traffic-alert-fieldset">
        <legend>{t("admin.trafficHijack.form.fields.alertChannels")}</legend>
        {["event", "debug", "email"].map((channel) => (
          <label className="console-traffic-check" key={channel}>
            <input
      ...
      <div className="console-traffic-form-actions">
        <Button
          disabled={disabled}
          icon={<ShieldAlert size={16} />}
          loading={loading}
          title={disabled ? permissionTitle : undefined}
          type="submit"
        >
          {editing
...
function TrafficOverviewCards({
  ...
  return (
    <div className="console-admin-stat-grid" aria-label={t("admin.trafficHijack.summary.label")}>
      <article
        className="console-admin-stat-card"
        data-state={abnormal > 0 ? "warning" : "success"}
      >
      ...
      <article className="console-admin-stat-card" data-state={abnormal > 0 ? "danger" : "success"}>
        <span aria-hidden="true">
          <Bell size={19} />
        </span>
        <div>
      ...
      <article className="console-admin-stat-card" data-state="info">
        <span aria-hidden="true">
          <RefreshCw size={19} />
        </span>
        <div>
...
function StatusBadge({ status }: { status: string }) {
  return (
    <Badge className="console-traffic-status-badge" data-status={status || "pending"}>
      {statusLabel(status)}
    </Badge>
  );
}
...
function SeverityBadge({ severity }: { severity: string }) {
  return (
    <Badge className="console-traffic-severity-badge" data-severity={severity || "ok"}>
...
function tableString(value: unknown, fallback = "") {
