...
const themeModeValuesSchema = z.object({
  dark: z.record(z.string().regex(/^--console-[a-z0-9-]+$/), safeThemeValueSchema),
  light: z.record(z.string().regex(/^--console-[a-z0-9-]+$/), safeThemeValueSchema),
});
...
export const consoleThemePackageSchema = z
  .object({
    assets: z
      .object({
    ...
    recipes: z.record(
      ...
      z
        .object({
          description: z.string().min(1),
          label: z.string().min(1),
          tokens: z.array(z.string().regex(/^--console-[a-z0-9-]+$/)),
        })
        .strict(),
    ),
    templates: z.object(
    ...
    tokens: z
      .object({
        ...
        tailwind: z.record(z.string().regex(/^--color-console-[a-z0-9-]+$/), safeThemeValueSchema),
...
export type ConsoleThemePackage = z.infer<typeof consoleThemePackageSchema>;
export type ConsoleThemeTemplateKey = (typeof requiredThemeTemplateKeys)[number];
export type ThemeChartPalette = ConsoleThemePackage["tokens"]["chartPalette"]["light"];
