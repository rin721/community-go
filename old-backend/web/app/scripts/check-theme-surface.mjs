...
const tokensPath = join(appDir, "components", "console", "tokens", "tokens.css");
const tokenSource = readFileSync(tokensPath, "utf8");
const definedVariables = new Set(
  [...tokenSource.matchAll(/(--console-[a-z0-9-]+)\s*:/g)].map((match) => match[1]),
);
const allowedUndeclared = new Set([
  "--console-media-category-depth",
...
for (const file of sourceFiles) {
  ...
  for (const match of source.matchAll(/var\((--console-[a-z0-9-]+)/g)) {
...
if (
  ...
  if (undefinedVariables.length > 0) {
    console.error("Undefined theme variables:");
...
console.log("Theme surface check passed.");
...
function shouldScan(file) {
  ...
  if (normalized.endsWith("/components/console/tokens/tokens.css")) {
    return false;
  }
