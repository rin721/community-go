...
export function ThemeDrawer({ open, theme, onChange, onReset, onClose }: { open: boolean; theme: ThemePreferences; onChange: (value: ThemePreferences) => void; onReset: () => void; onClose: () => void }) {
  ...
  const panelLabel = (value: ThemePanel) => translateMessage(`webui.host.theme.tab.${value}`);
  return <><button aria-label={translateMessage("webui.host.theme.close")} className={`drawer-backdrop ${open ? "visible" : ""}`} onClick={onClose} /><aside className={`theme-drawer ${open ? "open" : ""}`} aria-hidden={!open} role="dialog" aria-modal="true"><div className="drawer-header"><div><span className="drawer-kicker">{translateMessage("webui.host.theme.appearance")}</span><h2>{translateMessage("webui.host.theme")}</h2></div><button className="icon-button" onClick={onClose} aria-label={translateMessage("webui.host.theme.close")}><X size={18} /></button></div><nav className="theme-tabs" role="tablist" aria-label={translateMessage("webui.host.theme.tabs")}>
  ...
  </nav><div className="drawer-content">{panel === "appearance" && <div id="theme-panel-appearance" role="tabpanel"><ThemeSection title={translateMessage("webui.host.theme.mode")}><div className="theme-choice-grid">{modes.map(({ value, icon: Icon, label }) => <button type="button" key={value} className={theme.mode === value ? "theme-choice selected" : "theme-choice"} onClick={() => onChange({ ...theme, mode: value })}><Icon size={18} /><span>{translateMessage(label)}</span></button>)}</div></ThemeSection><ThemeSection title={translateMessage("webui.host.theme.density")}><div className="segmented-control">{densities.map((density) => <button type="button" key={density} className={theme.density === density ? "active" : ""} onClick={() => onChange({ ...theme, density })}>{translateMessage(`webui.host.theme.${density}`)}</button>)}</div></ThemeSection></div>}{panel === "layout" && <div id="theme-panel-layout" role="tabpanel"><ThemeSection title={translateMessage("webui.host.theme.layout.title")}><p className="theme-section-description">{translateMessage("webui.host.theme.layout.detail")}</p><ThemeSwitch label={translateMessage("webui.host.theme.layout.breadcrumb")} checked={theme.layout.showBreadcrumb} onChange={(checked) => onChange({ ...theme, layout: { ...theme.layout, showBreadcrumb: checked } })} /><ThemeSwitch label={translateMessage("webui.host.theme.layout.tabs")} checked={theme.layout.showTabs} onChange={(checked) => onChange({ ...theme, layout: { ...theme.layout, showTabs: checked } })} /><ThemeSwitch label={translateMessage("webui.host.theme.layout.footer")} checked={theme.layout.showFooter} onChange={(checked) => onChange({ ...theme, layout: { ...theme.layout, showFooter: checked } })} /><ThemeSwitch label={translateMessage("webui.host.theme.layout.sidebarCollapsed")} checked={theme.layout.sidebarCollapsed} onChange={(checked) => onChange({ ...theme, layout: { ...theme.layout, sidebarCollapsed: checked } })} /></ThemeSection></div>}{panel === "general" && <div id="theme-panel-general" role="tabpanel"><ThemeSection title={translateMessage("webui.host.theme.general.title")}><p className="theme-section-description">{translateMessage("webui.host.theme.general.detail")}</p><ThemeSwitch label={translateMessage("webui.host.theme.general.reduceMotion")} checked={theme.reduceMotion} onChange={(checked) => onChange({ ...theme, reduceMotion: checked })} /></ThemeSection></div>}{panel === "preset" && <div id="theme-panel-preset" role="tabpanel"><ThemeSection title={translateMessage("webui.host.theme.primary")}><p className="theme-section-description">{translateMessage("webui.host.theme.preset.detail")}</p><div className="color-preset-grid">{presets.map((preset) => <button type="button" key={preset} className={theme.preset === preset ? `color-preset ${preset} selected` : `color-preset ${preset}`} aria-label={translateMessage(`webui.host.theme.preset.${preset}`)} onClick={() => onChange({ ...theme, preset })}><span /></button>)}</div></ThemeSection></div>}</div><div className="drawer-footer"><Button variant="secondary" onClick={onReset}><RotateCcw size={16} />{translateMessage("webui.host.theme.reset")}</Button></div></aside></>;
}
