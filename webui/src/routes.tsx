...
import { isWorkspaceEligible, useWorkspaceHost } from "./workspace/WorkspaceProvider";
...
// RouteSlot 是 app 路由在普通路由树中的槽位。普通 route 直接渲染唯一的 Outlet；
// 显式 workspace route 只在宿主导航 effect 完成打开/激活前显示骨架，真实内容由
// WorkspaceOutlet 的 mounted panel 承载。槽位本身不创建标签，避免关闭标签后被渲染
// 循环重新打开。
...
export function RouteSlot({ route, manifest }: { route: ManifestRoute; manifest: Manifest }) {
  ...
  if (!isWorkspaceEligible(route)) return <ManifestRouteView route={route} manifest={manifest} />;
  // 已打开（含恢复/导航打开成功）时由 AppShell 决定渲染 panels；槽位只作为
  // 打开动作前的骨架/上限引导兜底，不让槽位重新创建标签。
  if (host.tabs.some((tab) => tab.routeID === route.id)) return <PageSkeleton />;
...
// renderAppRoutes 渲染 app 布局路由；普通路由由 RouteSlot 直接渲染，显式 workspace
// 路由的真实内容由 mounted panel 承载。带 groupLayoutId 的一族路由由共享模块布局
...
// renderPanelRoutes 是 WorkspaceOutlet 面板使用的 route 树：只包含显式 workspace route，
...
export function renderPanelRoutes(manifest: Manifest) {
  const workspaceRoutes = manifest.routes.filter((route) => route.layout === "app" && isWorkspaceEligible(route));
  const grouped = new Map<string, ManifestRoute[]>();
  const standalone: ManifestRoute[] = [];
