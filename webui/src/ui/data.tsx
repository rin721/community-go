...
export function DataTable<Row>({ columns, rows, ariaLabel, getRowKey = (_row, index) => String(index), loading = false, loadingLabel, emptyState, selectable = false, selectionLabel, selectedKeys, onSelectedKeysChange, wrapperProps, enhancements }: DataTableProps<Row>) {
  ...
  return <Table.Root className="data-table-wrap" data-ui-layer="component" data-ui-pattern="data-table" data-surface-owner="parent" data-density={density} data-sticky={stickyHeader || undefined} {...wrapperProps}>
    ...
    <Table.Content className="data-table" aria-label={ariaLabel} aria-busy={loading} selectionMode={selectable ? "multiple" : "none"} selectedKeys={selectable ? selected : undefined} onSelectionChange={(keys) => { if (!onSelectedKeysChange) return; onSelectedKeysChange(keys === "all" ? new Set(rowKeys) : new Set([...keys].map(String))); }}>
      <Table.Header>{selectable && <Table.Column id="__select" aria-label={selectionLabel}><Check slot="selection" checked={allSelected} indeterminate={partiallySelected} onChange={toggleAll} ariaLabel={selectionLabel} /></Table.Column>}{visibleColumns.map((column, index) => <Table.Column id={column.id} className={column.className} isRowHeader={index === 0} key={column.id}>{column.header}</Table.Column>)}{hasRowMenu && <Table.Column id="row-menu" aria-label={columnMenuLabel ?? ""} style={{ width: 104 }}>{rowMenuHeader}</Table.Column>}</Table.Header>
      <Table.Body>
        ...
        {!loading && rows.map((row, index) => { const key = getRowKey(row, index); return <Table.Row id={key} key={key}>{selectable && <Table.Cell><Check slot="selection" checked={selected.has(key)} onChange={() => toggleKey(key)} ariaLabel={selectionLabel} /></Table.Cell>}{visibleColumns.map((column) => <Table.Cell className={column.className} key={column.id}>{column.cell(row, index)}</Table.Cell>)}{hasRowMenu && <Table.Cell><DataTableRowMenu row={row} index={index} renderRowMenu={renderRowMenu!} /></Table.Cell>}</Table.Row>; })}
      </Table.Body>
    </Table.Content>
  </Table.Root>;
}
