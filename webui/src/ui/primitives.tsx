...
export function Check({ children, checked, onChange, disabled, className = "", indeterminate = false, ariaLabel, slot }: { children?: ReactNode; checked: boolean; onChange?: (checked: boolean) => void; disabled?: boolean; className?: string; indeterminate?: boolean; ariaLabel?: string; slot?: string }) {
  return <RACCheckbox slot={slot} isSelected={checked} isIndeterminate={indeterminate} isDisabled={disabled} aria-label={ariaLabel} onChange={(next) => onChange?.(next)} className={`rac-checkbox ${className}`.trim()}>{children ?? <span className="sr-only">{ariaLabel}</span>}</RACCheckbox>;
}
